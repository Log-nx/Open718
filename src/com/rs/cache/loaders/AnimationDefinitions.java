...
public class AnimationDefinitions {
	...
	public static void main(String[] args) throws IOException {
		Cache.init();
		int emoteId = 24944;
		AnimationDefinitions defs = getAnimationDefinitions(emoteId);
		int itemId = defs.leftHandItem == -1 || defs.leftHandItem == 65535 ? defs.rightHandItem : defs.leftHandItem;
		System.out.println(itemId);
		for (int i = 0; i < Utils.getAnimationDefinitionsSize(); i++) {
			defs = getAnimationDefinitions(i);
			if (defs == null)
				continue;
			if ((defs.leftHandItem == 16977 || defs.rightHandItem == 16977))
				System.out.println("anim:" + i + " uses right hand item=" + defs.rightHandItem
						+ ", uses left hand item=" + defs.leftHandItem + ", " + defs.clientScriptData);
		}
		for (int i = 0; i < Cache.RS3CACHE.getIndexes()[2].getValidFilesCount(32); i++) {
			RenderAnimDefinitions rdefs = RenderAnimDefinitions.getRenderAnimDefinitions(i);
			if (rdefs.defaultStandAnimation >= 13754 - 10 && rdefs.defaultStandAnimation <= 13754 + 10)
				System.out.println(i + " " + rdefs.defaultStandAnimation);
		}
	}
	...
	public static final AnimationDefinitions getAnimationDefinitions(int emoteId) {
		try {
			...
			if (defs != null)
				return defs;
			byte[] data = Cache.RS3CACHE.getIndexes()[20].getFile(emoteId >>> 7, emoteId & 0x7f);
	...
	private void readValues(InputStream buffer, int opcode) {
		if (opcode == 1) {
			...
			int i_7_ = opcode == 12 ? buffer.readUnsignedByte() : buffer.readUnsignedShort();
			...
			if (opcode == 19 || opcode == 119) {
				...
				anIntArray1362[opcode == 19 ? buffer.readUnsignedByte() : buffer.readUnsignedShort()] = buffer
						.readUnsignedByte();
				...
				int slot = opcode == 20 ? buffer.readUnsignedByte() : buffer.readUnsignedShort();
				soundMaxDelay[slot] = buffer.readUnsignedShort();
				soundMinDelay[slot] = buffer.readUnsignedShort();
			} else if (22 == opcode)
	 ...
	 * 0xffffffff) == -2) { int i = stream.readUnsignedShort(); anIntArray2153 = new
	 * int[i]; for (int i_16_ = 0; (i ^ 0xffffffff) < (i_16_ ^ 0xffffffff); i_16_++)
	 * anIntArray2153[i_16_] = stream.readUnsignedShort(); anIntArray2139 = new
	 * int[i]; for (int i_17_ = 0; (i_17_ ^ 0xffffffff) > (i ^ 0xffffffff); i_17_++)
	 * anIntArray2139[i_17_] = stream.readUnsignedShort(); for (int i_18_ = 0; i_18_
	 * < i; i_18_++) anIntArray2139[i_18_] = ((stream.readUnsignedShort() << 16) +
	 * anIntArray2139[i_18_]); } else if ((opcode ^ 0xffffffff) != -3) { if ((opcode
	 * ^ 0xffffffff) != -4) { if ((opcode ^ 0xffffffff) == -5) aBoolean2152 = true;
	 * else if (opcode == 5) anInt2142 = stream.readUnsignedByte(); else if (opcode
	 * != 6) { if ((opcode ^ 0xffffffff) == -8) leftHandItem =
	 * stream.readUnsignedShort(); else if ((opcode ^ 0xffffffff) != -9) { if
	 * (opcode != 9) { if ((opcode ^ 0xffffffff) != -11) { if ((opcode ^ 0xffffffff)
	 * == -12) anInt2155 = stream.readUnsignedByte(); else if (opcode == 12) { int i
	 * = stream.readUnsignedByte(); anIntArray2151 = new int[i]; for (int i_19_ = 0;
	 * ((i_19_ ^ 0xffffffff) > (i ^ 0xffffffff)); i_19_++) anIntArray2151[i_19_] =
	 * stream.readUnsignedShort(); for (int i_20_ = 0; i > i_20_; i_20_++)
	 * anIntArray2151[i_20_] = ((stream.readUnsignedShort() << 16) +
	 * anIntArray2151[i_20_]); } else if ((opcode ^ 0xffffffff) != -14) { if (opcode
	 * != 14) { if (opcode != 15) { if (opcode == 16) aBoolean2158 = true; // added
	 * opcode else if (opcode == 17) {
	 * 
	 ...
	 * int[handledSounds.length]; for (int index = 0; index < handledSounds.length;
	 * index++) anIntArray1362[index] = 255; }
	 * anIntArray1362[stream.readUnsignedByte()] = stream.readUnsignedByte(); //
	 * added opcode } else if (opcode == 20) { if ((soundMaxDelay == null) ||
	 * (soundMinDelay == null)) { soundMaxDelay = (new int[handledSounds.length]);
	 * soundMinDelay = (new int[handledSounds.length]); for (int i_34_ = 0; (i_34_ <
	 ...
	 * stream.readUnsignedByte(); //unused } else if (opcode == 24) { //rs3 added
	 * stream.readUnsignedShort(); //idk, calls for data from cache. idx 2, 77 }
	 * else if (opcode == 249) { int length = stream.readUnsignedByte(); if
	 * (clientScriptData == null) clientScriptData = new HashMap<Integer,
	 * Object>(length); for (int index = 0; index < length; index++) { boolean
	 * stringInstance = stream.readUnsignedByte() == 1; int key =
	 * stream.read24BitInt(); Object value = stringInstance ? stream.readString() :
	 * stream.readInt(); clientScriptData.put(key, value); } }
	 * 
	 * } else aBoolean2159 = true; } else aBoolean2141 = true; } else { // opcode 13
	 * int i = stream.readUnsignedShort(); handledSounds = new int[i][]; for (int
	 * i_21_ = 0; i_21_ < i; i_21_++) { int i_22_ = stream.readUnsignedByte(); if
	 * ((i_22_ ^ 0xffffffff) < -1) { handledSounds[i_21_] = new int[i_22_];
	 * handledSounds[i_21_][0] = stream.readUnsignedShort(); for (int i_23_ = 1;
	 * ((i_22_ ^ 0xffffffff) < (i_23_ ^ 0xffffffff)); i_23_++) {
	 * handledSounds[i_21_][i_23_] = stream.readUnsignedShort(); } } } } } else
	 * anInt2162 = stream.readUnsignedByte(); } else anInt2140 =
	 * stream.readUnsignedByte(); } else anInt2136 = stream.readUnsignedByte(); }
	 * else rightHandItemId = stream.readUnsignedShort(); } else { aBooleanArray2149
	 * = new boolean[256]; int i = stream.readUnsignedByte(); for (int i_24_ = 0; (i
