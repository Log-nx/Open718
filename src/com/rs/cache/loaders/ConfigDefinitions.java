...
public final class ConfigDefinitions {
	...
	public static final void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 20000; i++) {
			...
			if (cd.configId == SEARCHING_FILE_FOR_CONFIG)
				System.out.println("file: " + i + ", from bitshift:" + cd.anInt2024 + ", till bitshift: "
						+ cd.anInt2021+", "+cd.configId);
		}
		
		ConfigDefinitions cd = getConfigDefinitions(1549);
		System.out.println(" from bitshift:" + cd.anInt2024 + ", till bitshift: "
				+ cd.anInt2021+", "+cd.configId);
	...
	public static final ConfigDefinitions getConfigDefinitions(int id) {
		...
		if (script != null)//open new txt document
			return script;
		byte[] data = Cache.RS3CACHE.getIndexes()[22].getFile(id >>> 1416501898,
				id & 0x3ff);
