...
public class IComponentDefinitions {
    ...
    public static void main(String[] args) throws IOException {
        System.out.println("component id size is " + (Utils.getInterfaceDefinitionsComponentsSize(884)));//which interface u want to find scripts for? I forgot skills interid
        IComponentDefinitions[] defs = getInterface(884);// where is the cs2 compiler? gotta remember
    }

    public static Integer[] getSpriteIds(int interfaceId) {
        List<Integer> ids = new ArrayList<Integer>();
        IComponentDefinitions[] defs = getInterface(interfaceId);
        for (IComponentDefinitions d : defs) {
            if (d != null && d.spriteId != -1 && !ids.contains(d.spriteId))
                ids.add(d.spriteId);
    ...
    public static Integer[] getModelIds(int interfaceId) {
        List<Integer> ids = new ArrayList<Integer>();
        IComponentDefinitions[] defs = getInterface(interfaceId);
        for (IComponentDefinitions d : defs) {
            if (d != null && d.modelId != -1 && !ids.contains(d.modelId))
                ids.add(d.modelId);
    ...
    public static IComponentDefinitions[] getInterface(int id) {
        ...
        if (icomponentsdefs[id] == null) {
            ...
            for (int i = 0; i < icomponentsdefs[id].length; i++) {
                byte[] data = Cache.RS3CACHE.getIndexes()[3].getFile(id, i);
                if (data != null) {
    ...
    final void decode(InputStream stream) {
        ...
        if (hidden) {
            System.err.println("hidden [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: true");
        }
        if (newInt >= 0) {
            aBoolean4858 = (i_17_ & 0x2 ^ 0xffffffff) != -1;
            if (aBoolean4858)
                System.err.println("hidden 1 [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: true");
        }
        if (type == 0) {
        ...
        if (type == 4) {
            ...
            /*
             * else { anInt4759 = stream.readUnsignedShort(); if ((anInt4759 ^ 0xffffffff)
             * == -65536) { anInt4759 = -1; } }
             */
            ...
            System.err.println("text [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]:" + aString4790);
        ...
        if (aStringArray4686 != null) {
            System.err.println("has options [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + Arrays.toString(aStringArray4686));
        }
        int i_28_ = i_25_ >> 4;
        ...
        if (anIntArray4863 != null) {
            System.err.println("has music [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + Arrays.toString(anIntArray4863));
        }
        ...
        if (aString4784 != null)
            System.err.println("hidden string [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + aString4784);
        ...
        if ((method925(i_20_) ^ 0xffffffff) != -1) {
            ...
            System.err.println("has settings [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + i_32_ + ", " + anInt4698 + ", " + anInt4839);
        ...
        if (newInt >= 0) {
            ...
            for (int i_34_ = 0; i_33_ > i_34_; i_34_++) {
                ...
                System.err.println("has unknown [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + (long) i_35_ + ", " + i_36_);
            }
            ...
            for (int i_38_ = 0; i_38_ < i_37_; i_38_++) {
                ...
                System.err.println("has unknown [" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]: " + (long) i_39_ + ", " + string);
            }

        }

        anObjectArray4758 = decodeScript(stream, true);
        ...
        anObjectArray4834 = decodeScript(stream, true);
        anObjectArray4774 = decodeScript(stream, true);
        anObjectArray4803 = decodeScript(stream, true);
        anObjectArray4680 = decodeScript(stream, true);
        anObjectArray4856 = decodeScript(stream, true);
        anObjectArray4852 = decodeScript(stream, true);
        anObjectArray4711 = decodeScript(stream, true);
        anObjectArray4753 = decodeScript(stream, true);
        anObjectArray4688 = decodeScript(stream);
        anObjectArray4775 = decodeScript(stream);
        triggers1 = method4150(stream);
        triggers2 = method4150(stream);
        triggers3 = method4150(stream);
        triggers4 = method4150(stream);
        triggers5 = method4150(stream);
    }
    ...
    private final Object[] decodeScript(InputStream buffer) {
        ...
        hasScripts = true;
        System.err.println("Call found[" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]:" + Arrays.toString(objects));
        return objects;
    }

    private final Object[] decodeScript(InputStream buffer, boolean text) {
        int i = buffer.readUnsignedByte();
        if ((i ^ 0xffffffff) == -1) {
            return null;
        }
    ...
    private final int[] method4150(InputStream buffer) {
        ...
        System.err.println("Trigger found[" + (ihash >> 16) + "," + (ihash & 0xFFF) + "]:" + Arrays.toString(is));
        return is;
    }
