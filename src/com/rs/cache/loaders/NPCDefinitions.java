...
public final class NPCDefinitions {
	...
	public static final NPCDefinitions getNPCDefinitions(int id) {
		...
		if (def == null) {
			...
			def.method694();
			byte[] data = Cache.RS3CACHE.getIndexes()[18].getFile(
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 26023; i++) {
			...
			/*if (status != null && status.equals(status2)) {
				...
				System.out.println(i + ", " + status + ", " + status2 + ", " + def.name);
			}*/
			NPC npc = new NPC(def.id, new WorldTile(0, 0, 0), 0, false, false);
			String id = String.valueOf(def.id);
			String name = def.name;
			String examine = NPCExamines.getExamine(npc);
			System.out.println(def.id + " " + def.name + NPCExamines.getExamine(npc));
		}

	}
	
	public static void findNPC(Player p, String name) {
		for (int i = 0; i < Utils.getNPCDefinitionsSize(); i++) {
			NPCDefinitions defs = NPCDefinitions.getNPCDefinitions(i);
			if (defs != null) {
				if (defs.name.toLowerCase().contains(name)) {
					StringBuilder sb = new StringBuilder();
					p.getPackets()
							.sendPanelBoxMessage(defs.name + " <br>ID: " + defs.id + " <br>Level: "
									+ defs.combatLevel + " <br>Size: " + defs.size + " <br>Options: " + sb.toString()
									+ "<br><br></col>");
				}
			}
		}
																			...
																			if ((opcode ^ 0xffffffff) != -140) {
																				if (opcode == 140)
																					...
																					if (opcode == 143)
																						...
																						stream.readSmart3();
																						stream.readSmart3();
																					} else if (opcode == 180) {// new
																						int anInt5809 = stream
																								.readUnsignedByte();
																					} else if (opcode == 181) {// new
																						short aShort5806 = (short) stream
																								.readUnsignedShort();
																						byte aByte5847 = (byte) stream
																								.readUnsignedByte();
																					} else if (182 == opcode) {
																						boolean aBool5871 = true;
																					} else if (opcode == 249) {
																						int i = stream
																								.readUnsignedByte();
	...
	public boolean hasAttackOption() {
		...
		for (String option : options) {
			if (option != null
					&& (option.equalsIgnoreCase("attack") || option
