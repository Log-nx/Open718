...
public final class VarBitDefinitions {
	...
	Cache.init();
	System.out.println("There are currently: " + Cache.RS3CACHE.getIndexes()[22].getLastArchiveId() * 0x3ff + " bitConfigs.");
	// List<BitConfigDefinitions> configs = new
	// ArrayList<BitConfigDefinitions>();
	for (int i = 0; i < Cache.RS3CACHE.getIndexes()[22].getLastArchiveId() * 0x3ff; i++) {
	...
	if (script != null)// open new txt document
	    return script;
	byte[] data = Cache.RS3CACHE.getIndexes()[22].getFile(id >>> 1416501898, id & 0x3ff);
