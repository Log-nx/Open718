		...
		for (int dynZ = 0; dynZ < 4; dynZ++) {
			for (int dynX = 0; dynX < 8; dynX++) {
				for (int dynY = 0; dynY < 8; dynY++) {
					...
					byte[] mapContainerData = Cache.RS3CACHE.getIndexes()[5].getFile(archiveId, 3);
					byte[] landContainerData = Cache.RS3CACHE.getIndexes()[5].getFile(archiveId, 0);
								...
								for (int y = 0; y < 64; y++) {
									if ((mapSettings[z][x][y] & 0x1) == 1) {
										...
										if (realZ == renderChunkZ && (x >> 3) == renderLocalChunkX && (y >> 3) == renderLocalChunkY) {
											...
											forceGetRegionMap().addFloor(dynZ, (dynX << 3) | coords[0], (dynY << 3) | coords[1]);//XXX change to addUnwalkable.
										}
									}
								}
							}
