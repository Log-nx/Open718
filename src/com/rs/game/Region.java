...
public class Region {
	...
	private HashMap<String, Integer[]> entityclipping;
	...
	public void spawnObject(Player player, WorldObject object, int plane, int localX, int localY, boolean original) {
		if (objects == null)
			objects = new WorldObject[4][64][64][4];
		int slot = OBJECT_SLOTS[object.getType()];
		if (original) {
			...
			clip(object, localX, localY);
			if (player == null || player.hasFinished() || !player.getMapRegionsIds().contains(regionId))
				return;
	...
	public void removeObject(Player player, WorldObject object, int plane, int localX, int localY) {
		if (objects == null)
			objects = new WorldObject[4][64][64][4];
		int slot = OBJECT_SLOTS[object.getType()];
		WorldObject removed = getRemovedObjectWithSlot(plane, localX, localY, slot);
		...
		if (!player.getMapRegionsIds().contains(regionId)) {
			System.out.println("WE STOP REMOVING OMG THIS FUCKER REGION");
			return;
		}
		if (original != null)
			player.getPackets().sendSpawnedObject(original);
		else
			player.getPackets().sendDestroyObject(object);

	}

	public void removeObject(Player player, WorldObject object, int plane, int localX, int localY, boolean removeClip) {
		if (objects == null)
			objects = new WorldObject[4][64][64][4];
		int slot = OBJECT_SLOTS[object.getType()];
		WorldObject removed = getRemovedObjectWithSlot(plane, localX, localY, slot);
	...
	public void loadRegionMap() {
		...
		byte[] mapSettingsData = Cache.RS3CACHE.getIndexes()[5].getFile(archiveId, 3);
		byte[][][] mapSettings = loadMapSettings(mapSettingsData);
		if (mapSettingsData == null)
			return;

		byte[] objectsData = Cache.RS3CACHE.getIndexes()[5].getFile(archiveId, 0);
		if (objectsData != null)
	...
	public byte[][][] loadMapSettings(byte[] data) {
		...
		if (data != null) {
					...
					for (int y = 0; y < 64; y++) {
						if ((mapSettings[plane][x][y] & 0x1) == 1) {
							...
							if ((mapSettings[1][x][y] & 2) == 2)
								realPlane--;
							if (realPlane >= 0)
								forceGetRegionMap().addFloor(realPlane, x, y);
						}
					}
				}
			}
			...
			for (int plane = 0; plane < 4; plane++) {
				for (int x = 0; x < 64; x++) {
					for (int y = 0; y < 64; y++) {
						forceGetRegionMap().addFloor(plane, x, y);
					}
				}
			}
		}
	...
	public void loadMapObjects(byte[] data, int regionX, int regionY, byte[][][] mapSettings) {
		...
		while ((incr = landStream.readSmart2()) != 0) {
			...
			while ((incr2 = landStream.readUnsignedSmart()) != 0) {
				...
				int type = objectData.type;
				int rotation = objectData.rotation;
				...
				if (objectPlane < 0 || objectPlane >= 4 || plane < 0 || plane >= 4)
					continue;
				spawnObject(new WorldObject(objectId, type, rotation, localX + regionX * 64, localY + regionY * 64,
	...
	public void handleEntityClipping(String name, Integer[] integers, boolean add) {
		if (entityclipping == null) {
			entityclipping = new HashMap<String, Integer[]>();
		}
		if (!add) {
			entityclipping.remove(name);
		} else {
			entityclipping.put(name, integers);
		}
	}

	public List<Integer[]> getEntityClipping(String name) {
		if (getLoadMapStage() != 2)
			return null; // cliped tile
		List<Integer[]> toReturn = new ArrayList<Integer[]>();
		if (entityclipping == null)
			return toReturn;
		for (String storedname : entityclipping.keySet()) {
			if (name.equals(storedname))
				continue;
			toReturn.add(entityclipping.get(storedname));
		}
		return toReturn;
	}
}