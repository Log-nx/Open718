...
public final class RegionBuilder {
	...
	for (int mapX = 0; mapX < MAX_REGION_X; mapX++) {
		...
		if (Cache.RS3CACHE.getIndexes()[5].getArchiveId("m" + mapX + "_" + mapY) != -1)
	...
	public static void entityClip(String name, final int x, final int y, final int plane, int size, int follower, boolean add) {
		Integer[] tile = new Integer[] {x, y, plane, size, follower};
		int regionId = new WorldTile(x, y, plane).getRegionId();
		Region region = World.getRegion(regionId);
		region.handleEntityClipping(name, tile, add);
	}
	
	public static List<Integer[]> getEntityClip(String name, final int x, final int y, final int plane) {
		int regionId = new WorldTile(x, y, plane).getRegionId();
		Region region = World.getRegion(regionId);
		if (region == null)
			return null;
		return region.getEntityClipping(name);
	}
}