...
public abstract class BossInstance {
	...
	public void enterInstance(Player player, boolean login) {
		synchronized (BossInstanceHandler.LOCK) {
			...
			playMusic(player);
			if (!isPublic())
				player.getPackets().sendGameMessage("Welcome to this session against <col=00FFFF>" + getInstanceName()
						+ "</col>. This arena will expire in " + (settings.getTimeRemaining() / 60000) + " minutes.");
			if (!login) {
				player.getControllerManager().removeControllerWithoutCheck();
				player.getControllerManager().startController(settings.getBoss().getControllerName(), this);
			} else {
