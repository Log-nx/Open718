...
public final class CastleWars {
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		for (int i = 0; i < playing.length; i++)
			...
			if (winner != -1) {
				...
				player.getInventory().addItemDrop(CW_TICKET, 1);
			    } else if (winner == i) {
				player.getPackets().sendGameMessage("You won.");
					player.getInventory().addItemDrop(CW_TICKET, 2);
	...
	player.getInterfaceManager().closeInterface(57, 57);
	...
	public void reset() {
	    ...
	    World.removeObject(object, object.getId() == 4377 || object.getId() == 4378 ? false : true);
	...
	private void makeSafe(int flagTeam) {
	    ...
	    if (flagStand == null)
		return;
	    World.removeObject(flagStand, false);
