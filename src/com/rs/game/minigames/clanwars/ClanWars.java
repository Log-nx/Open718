...
public final class ClanWars implements Serializable {
	...
	public static void enter(Player p) {
		...
		p.getInterfaceManager().sendTab(
				p.getInterfaceManager().hasResizableScreen() ? 11 : 0, 265);
	...
	public void endWar() {
		...
		for (Player p : viewers) {
			...
			p.getPackets().closeInterface(
					p.getInterfaceManager().hasResizableScreen() ? 11 : 0);
			p.getControllerManager().startController("clan_wars_request");
	...
	public void switchRule(Rules rule, Player player) {
		...
		if (rule == Rules.NO_MAGIC) {
			if (get(Rules.NO_RANGE) && get(Rules.NO_MELEE)) {
				player.getPackets()
						.sendGameMessage(
								"You can't deactivate all combat style rules, how would you fight?");
				return;
			} else {
