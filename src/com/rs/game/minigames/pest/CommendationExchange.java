...
public class CommendationExchange {// 1875 TODO
	 ...
	 * Void related stuff
	 */
	private static final int[] VOID_BASE_COMPONENTS = { 15, 196, 208, 220, 232,
			244, 256, 268, 280 };
	private static final int[] VOID = { 11676, 11675, 11674, 8839, 8840, 8842,
			8841, 19711, 11666 };
	private static final int[] VOID_POINTS_COST = { 200, 200, 200, 250, 250,
			150, 250, 150, 10 };
	...
	private static double calculateExperience(Player player, int skill) {
		...
		if (skill == Skills.MAGIC || skill == Skills.RANGE)
			...
			constant = 18;
		return (Math.ceil(((level + 25) * (level - 24)) / 606) * constant)
				+ constant;
	}
	...
	private static boolean exchangeCommendation(Player player, int price) {
		...
		if (currentPoints - price < 0) {
			player.getPackets()
					.sendGameMessage(
							"You don't have enough Commendations remaining to complete this exchange.");
			return false;
		}
