...
public class Lander {
    ...
    private static final int AUTO_GAME = Settings.DEBUG ? 1 : 1;
	...
	public void run() {
		try {
			if (seconds == 60 && lobby.size() >= 2 || lobby.size() >= AUTO_GAME) {
				passPlayersToGame();
			} else if (seconds == 0)
				seconds = TIME;
    ...
    public static enum LanderRequirement {
		...
		NOVICE(0, 40, new WorldTile(2653, 2634, 0), new WorldTile(2650, 2634, 0)),

		INTERMEDIATE(1, 70, new WorldTile(2640, 2631, 0), new WorldTile(2637, 2631, 0)),

		VETERAN(2, 100, new WorldTile(2629, 2645, 0), new WorldTile(2632, 2645, 0));
