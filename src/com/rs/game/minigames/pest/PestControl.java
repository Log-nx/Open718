...
public class PestControl {
	...
	public enum PestData {
		...
		NOVICE(new int[] { /* Shifters */3732, 3733, 3734, 3735, /* Ravagers */
				3742, 3743, 3744, /* Brawler */3772, 3773, /* Splatter */3727,
				3728, 3729, /* Spinner */3747, 3748, 3749, /* Torcher */3752,
				3753, 3754, 3755, /* Defiler */3762, 3763, 3764, 3765 },
	...
	public boolean createPestNPC(int index) {
		...
		for (int trycount = 0; trycount < 10; trycount++) {
			...
			if (World.canMoveNPC(baseTile.getPlane(), teleTile.getX(), teleTile.getY(), defs.size))
				break;
			teleTile = baseTile;
		}
	...
	private void sendBeginningWave() {
		knight = new PestPortal(KNIGHT_IDS[Utils.random(KNIGHT_IDS.length)], true, getWorldTile(33, 34), this);
		knight.unlock();
	...
	private void sendFinalReward(Player player, int knightZeal) {
		if (knight.isDead())
			...
			player.getDialogueManager().startDialogue("SimpleMessage", "Congratulations! You have successfully kept the lander safe and have been awarded: "
			+ coinsAmount + " gold coins and " + pointsAmount + " commendation points.");
			player.getMoneyPouch().sendDynamicInteraction(coinsAmount, false);
