...
public class AraxyteNPC extends NPC {
    ...
    public void processNPC() {
			    ...
			    public void run() {
					...
					if (step == 10) {
						for (Player player : AraxyteHyveController.getPlayers()) {
							player.getPackets().sendGameMessage(Colors.DARK_RED+"Araxxor prepares to come down from the ceiling and charge!");
						}
					}
					if (step == 15) {
						setHitpoints(getMaxHitpoints());
						setNextWorldTile(new WorldTile(4592, 6266, 1));
		...
		if (getHitpoints() <= 10000 && !AraxyteNPC.heal2) { // second heal
			...
			WorldTasksManager.schedule(new WorldTask() {
			    ...
			    public void run() {
			    	if (step == 0) {
			    		applyHit(new Hit(null, 2500, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 2) {
			    		applyHit(new Hit(null, 2750, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 4) {
			    		applyHit(new Hit(null, 3000, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 6) {
			    		applyHit(new Hit(null, 3250, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 8) {
		...
		if (getHitpoints() <= 25000 && !heal1) { // first heal
			...
			WorldTasksManager.schedule(new WorldTask() {
			    ...
			    public void run() {
			    	if (step == 0) {
			    		applyHit(new Hit(null, 1250, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 2) {
			    		applyHit(new Hit(null, 1500, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 4) {
			    		applyHit(new Hit(null, 1750, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 6) {
			    		applyHit(new Hit(null, 2000, HitLook.HEALED_DAMAGE));
			    	}
			    	if (step == 8) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (loop >= defs.getDeathDelay() + 1) {
					new NPC(19464, lastWorldTile, -1, true, true);
					if (id == 19464 ) {
     ...
     */
 	...
 	private void sendObject(Player player) {
		...
		player.getPackets().sendGameMessage("You've killed a total of "+Colors.RED+player.increaseKillStatistics("araxxor", true)
	 			+ "</col> x "+Colors.RED+"Araxxor</col>.", true);
		for (Player all : AraxyteHyveController.getPlayers()) {
			if (all == null || all == player)
				continue;
			all.getPackets().sendGameMessage(Colors.DARK_RED+player.getDisplayName()+" has slain the Araxxor; "
					+ "his total Araxxor kills: "+player.increaseKillStatistics("araxxor", false)+".");
		}
		if (rewards != null) {
			Item barry = new Item(33809);
			if (!player.getInventory().contains(barry) && Utils.random(player.getPerkManager().hasPerk(PlayerPerks.PET_DROPS) ? 1875 : 2500) == 1) {
				rewards.add(barry);
				World.sendWorldMessage(Colors.ORANGE + "<shad=000000><img=6>News: "
	    				+ player.getDisplayName() + " received an Araxxor (Mallary) pet drop.", false, false);
				return;
			}
			...
			if (rewards.contains(new Item(31718))) {
				for (Player all : AraxyteHyveController.getPlayers()) {
					if (all == null || all == player)
						continue;
					all.getPackets().sendGameMessage(Colors.ORANGE + "<shad=000000><img=6>Araxyte cave: "
 	...
 	private void sendDropNews(Player player, Item item) {
 		World.sendWorldMessage(Colors.ORANGE + "<shad=000000><img=6>News: "
				+ player.getDisplayName() + " received "+Utils.getAorAn(item.getName()) + " "
						+ item.getName()+" from " + getName() + ".", false, false);
 	}
