 ...
 * Represents Araxxor.
 * @author Nosz & Jason
 *	World.spawnObject(new GameObject(91641, 22, 1, player.getX(), player.getY(), player.getPlane()), true); // use this for egg ability is phase 2
 */
public final class Araxxor extends NPC {
	 ...
	 */
	private static final AraxxorAttack[] PHASE_1_ATTACKS = { // new Cacoon() new RangeAttack(),  new MeleeAttack()  new Cleave()
		new MeleeAttack(),
			new MagicAttack(),
			new RangeAttack()
	 ...
	 */
	private static final AraxxorAttack[] PHASE_3_ATTACKS = {
			new MeleeAttack(),
			new MagicAttack(),
			new RangeAttack(),
			new EggAttack()
	 ...
	 */
	private static final AraxxorAttack[] PHASE_4_ATTACKS = { //Super fire attack twice, to make it more often.
			new MeleeAttack(), new RangeAttack(), 
			new WebShield(), new Cacoon(), new AcidicSpider()
	};
	 ...
	 * @param tile The world tile to set the spider on.
	 * @param base The dynamic region's base WorldTile.
	 */
	...
	public Araxxor(Player attacker, WorldTile tile, WorldTile base) {
		...
		this.nextAttack = 10;
		this.setCanBeAttackFromOutOfArea(false);
	...
	public void handleIngoingHit(Hit hit) {
		...
		if(player instanceof Player){
			...
			if(p.araxxorHeal){
				player.applyHit(new Hit(player, hit.getDamage(),HitLook.REFLECTED_DAMAGE));
			}
		}
	...
	public void sendDeath(Entity source) {
		if(attacker.AraxxorLastState == true && attacker.AraxxorThirdStage == false) {
		  //  increaseKills(RecordKey.ARAXXI, false);
		   // attacker.sendMessage("KILLCOUNT SENT");
		...
		} else if(attacker.AraxxorThirdStage == true && attacker.AraxxorLastState == false) {
			setCantInteract(false);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (time == 9) {
					npcLocked = true;
					Death();
					final WorldObject AraxxorBody = new WorldObject(91673,10, 0, AraxDeathX, AraxDeathY, AraxDeathZ); // object with animation that player will mine
					World.spawnTemporaryObject(AraxxorBody, 120000);	// time object will stay in miliseconds
					//World.spawnObject(new GameObject (91673, 10, 0, AraxDeathX, AraxDeathY, AraxDeathZ), true);
				//	World.spawnObject(AraxxorBody);
					attacker.sm("Araxxor's Corpse will only last 2 minutes.");
	...
	public void processNPC() {
		...
		if(this.getX() >= base.getX() + (187 - 64) && this.getX() <= base.getX() + (201-64)){
			platformStand++;
			if (platformStand == 3) {
				this.AcidLevel ++;
				...
				attacker.sm("Araxxor Acid Level: "+AcidLevel+"");
				platformStand = 0;
			}
		} else 	if (this.getX() >= base.getX() + (202-64) && this.getX() <= base.getX() + (216-64)) {
			if (this.AcidLevel >= 1) {
				if(this.RampedRemoved == 0) {
				World.removeObject(new WorldObject(91520, 10, 0, base.transform(135, 20, 0))); // burn option
				World.spawnObject(new WorldObject(91521, 10, 0, base.transform(135, 20, 0))); // Huge Ramp in center of map ( stage 0 no acid )
				this.RampedRemoved = 1;
				}
				attacker.sm("Ramp acid level: "+FloorAcidLevel+"");
				AcidLevel --;
				FloorAcidLevel ++;
					...
					if(this.RampedRemoved == 1) {
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									time ++;
									//attacker.sendMessage(""+time+"");
									if (time == 1 && RampedRemoved == 1) {
										switchState(SpiderState.MELEE);
										//attacker.sendMessage("acid floor 1");
										World.spawnObject(new WorldObject(91670, 10, 0, base.transform(217-64, 86-64, 0)));
										World.spawnObject(new WorldObject(91526, 10, 0, base.transform(135, 20, 0)));
										FloorAcidLevel = 0;
										RampedRemoved = 2;
										stop();
								}
		...
		if(ticks > nextAttack){
			canLoot = false;
			attacker.sm("An error as occurred and you will no longer receive loot. Please teleport out and try again.");
		}

		if (ticks == nextAttack) {
			if(npcLocked){
				//attacker.sendMessage("npc is paused and not attacking");
				return;
		}
			AraxxorAttack attack;
			if(attacker.AraxxorAttackCount < 6) {
				//attacker.sendMessage(""+attacker.AraxxorAttackCount+"");
				...
				if(!isLocked()){	
					if(npcLocked) {
						if(attackStage >= 0 && attackStage != 6) {
							...
							attackStage ++;
							//attacker.sendMessage("attack stage2: "+attackStage+"");
						} else {
							if(attackStage == 6) {
								attackStage = 0;
		...
		WorldTasksManager.schedule(new WorldTask() {			
			...
			public void run() {
				...
				if (time == 7) {
					...
					Araxxor.setNextWorldTile(base.transform(231-64, 90-64, 0));
				}
				if (time == 9) {
					setCantInteract(false);
					Araxxor.setNextAnimation(new Animation(24076));
	...
	public void startStageFour(final NPC Araxxor, final Player attacker) {
		WorldTasksManager.schedule(new WorldTask() {			
			...
			public void run() {
				time ++;
				if (time == 1) {
					Araxxor.setNextWorldTile(base.transform(252-64, 88-64, 0));	
					//finalWalk(Araxxor, attacker);
					//attacker.setNextWorldTile(base.transform(110, 38, 0));
					attacker.setNextWorldTile(base.transform(247-64, 86-64, 0));
				}
				if(time == 4) {
					//finalWalk(Araxxor, attacker);
					setCantInteract(true);
				...
				//	Araxxor.setHitpoints(getMaxHitpoints()); // used for final version
					Araxxor.setHitpoints(15000);
					//Araxxor.setNextWorldTile(base.transform(115, 38, 0));
					setCantInteract(false);
					setLocked(false);
					npcLocked = false;
	 ...
	 * @param needed this crap just to get the fucking spider to walk ughhh
	 */
		...
		if (distanceX < size && distanceX > -1 && distanceY < size 
				&& distanceY > -1 && !attacker.hasWalkSteps() && !hasWalkSteps()) {
			resetWalkSteps();
		...
		if ((!clipedProjectile(attacker, true)) || distanceX > size || distanceX < -1 || distanceY > size || distanceY < -1) {
			resetWalkSteps();
			addWalkStepsInteract(attacker.getX(), attacker.getY(), getRun() ? 2 : 1, size, true);
			return;
		}
		resetWalkSteps();
	}
	...
	public void setPhase(int phase) {
		...
		//	}
			...
			//attacker.getPackets().sendSpawnedObject(new GameObject(91673 , 10, 0, base.transform(80, 40, -1)));
			attacker.getPackets().sendGameMessage("<col=33FFFF>You have slain the evil spiders.</col>");
			break;
		}
	}
...
//	public List<Spiders> getSpiders() {
		...
		WorldTasksManager.schedule(new WorldTask() {	
			...
			public void run() {
				...
				if(target.AraxxorAttackCount < 6) {
					...
					target.setNextAnimation(new Animation(Combat.getDefenceEmote(target)));
					target.applyHit(new Hit(npc, hit, hit == 0 ? HitLook.MISSED : HitLook.MELEE_DAMAGE));
					}else {
	...
	public void cacoonTimer(final Player p, final NPC npc) {
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if(p.cacoonTime == 6) {
					p.cacoonTime = 0;
					//p.sendMessage("Cacoon Timer Has RESET");
					stop();
				}
	...
	public void spawnEggSpiders(Player p) {
		final WorldTile SpawnSpiders = base.transform(p.eggSpidersX, p.eggSpidersY, p.getPlane());
		...
		//npc1.setNextGraphics(new Graphics (4999));
		//Arraxi = npc1;
		//attacker.sendMessage("SPIDERS SENT");
