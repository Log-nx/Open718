...
public final class NPCCombat {
	...
	private int combatAttack() {
		...
		if ((!(npc instanceof Nex)) && (!(npc instanceof KalphiteKing)) && (!(npc instanceof Vorago)) && (!(npc instanceof Lucien)) && !npc.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) {
			return 0;
		}
	...
	public boolean checkAll() {
		...
		if(!npc.isCantFollowUnderCombat()) {
			...
			if (npc instanceof Nex) {
				...
				if (!nex.isFlying() && (!npc.clipedProjectile(target, maxDistance == 0
