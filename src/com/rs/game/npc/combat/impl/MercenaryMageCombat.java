...
public class MercenaryMageCombat extends CombatScript {
	...
	public int attack(final NPC npc, Entity target) {
		...
		if (attackStyle == 0) {
				...
				public void run() {
					for (Player player : World.getPlayers()) {
						...
						if (player.withinDistance(center, 3)) {
							...
							delayHit(npc, 0, player, new Hit(npc, Utils.random(100, 300), HitLook.REGULAR_DAMAGE));
						}
					}
				}
			}, 2);
			...
			delayHit(npc, 2, target, getMagicHit(npc, getRandomMaxHit(npc, Utils.random(100, 300), NPCCombatDefinitions.MAGE, target)));
