...
import com.rs.game.player.creation.Dragonfireshield;
...
public class RuneDragonCombat extends CombatScript {
	...
	private void rangeAttack(RuneDragon dragon, Entity target) {
		...
		if (Utils.random(3) == 0) {
			...
			if (target instanceof Player) {
				((Player) target).getPackets().sendGameMessage("You've been prevented from moving.", true);
	...
	private void dragonFireAttack(RuneDragon dragon, Entity target) {
		...
		if (target instanceof Player) {
			...
			if (damage > 0)
				player.getPackets().sendGameMessage("You are hit by the dragon's"+(dragon.getPhase() == 2 ? " enhanced" : "")
					+ " fiery breath!", true);
		}
