...
public class BeastOfBurden implements Serializable {
	...
	public void dropBob() {
		/*
		 * int size = familiar.getSize(); WorldTile WorldTile = new
		 * WorldTile(familiar.getCoordFaceX(size), familiar.getCoordFaceY(size),
		 * familiar.getPlane());
		 */
		/*
		 * for (int i = 0; i < beastItems.getSize(); i++) { Item item =
		 * beastItems.get(i); if (item != null) World.addGroundItem(item,
		 * WorldTile, player, false, -1, false); }
		 */
		beastItems.reset();
	}
