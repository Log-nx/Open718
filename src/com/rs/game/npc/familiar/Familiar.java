...
public abstract class Familiar extends NPC implements Serializable {
	 ...
	 */
    public static enum SpecialAttack { ITEM, ENTITY, CLICK, OBJECT }
     ...
     * @param mapAreaNameHash the map area name.
     * @param canBeAttackFromOutOfArea if can be attacked out of area.
     */
    public Familiar(Player owner, Pouches pouch, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
		...
		this.owner = owner;
		if (owner.getPerkManager().hasPerk(PlayerPerks.FAMILIAR_MASTER)) {
			hitpoints = getHitpoints();
			setHitpoints((int) (getHitpoints() + getHitpoints() * 0.25));
			owner.sendMessage(getName()+"'s hitpoints increased to "+getHitpoints()+" from "
					+ hitpoints + " thanks to Familiar Expert perk.");
		}
		this.pouch = pouch;
		resetTicks();
		specialEnergy = 60;
     ...
     * @param login if the owner has just logged in.
     */
     ...
     * @param logged if owner logging out.
     */
    public void dismissFamiliar(boolean logged) {
		finish();
    ...
    /**
     * Drains familiars special attack.
     * @param specialReduction how many points should we reduce.
     */
    ...
    public void processNPC() {
		...
		if (trackTimer == (owner.getPerkManager().hasPerk(PlayerPerks.FAMILIAR_MASTER) ? 75 : 50)) {
		    trackTimer = 0;
		    ticks--;
		    if (trackDrain) {
    ...
    public boolean renewFamiliar() {
		if (ticks > 5) {
		    ...
		    owner.sendMessage("You need a " + ItemDefinitions.getItemDefinitions(pouch.getPouchId())
					    .getName().toLowerCase() + " to renew your familiar's timer.");
		    return false;
		}
		resetTicks();
     ...
     * @param owner The owner as Player.
     */
     ...
     * @param energy by how much should we restore as Integer.
     */
		...
		WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
				if (loop == 0) {
				    ...
				    if (!owner.getPerkManager().hasPerk(PlayerPerks.FAMILIAR_MASTER)) {
						owner.sendMessage("Consider purchasing the Familiar Expert perk "
				    			+ "to increase Pet timers and health.");
					}
	 ...
	 * @param on if we're switching the orb on.
	 */
	 ...
	 * @param player The Owner as Player.
	 */
	 ...
	 * @param player The Owner as Player.
	 * @param summoningLeftClickOption the left-click option as Integer.
	 */
	...
	/**
	 * Unlocks the summoning orb based on the familiar on the Owners game client.
	 */
	public void unlock() {
	 ...
	 * @param on if we're turning the special attack on/off
	 */
     ...
     * @param object The object to execute the special attack on.
     * @return if can execute special attack.
     */
    public abstract boolean submitSpecial(Object object);
    
    public static void handleFamiliar(Player player, int componentId, int interfaceId) {
		...
		if (interfaceId == 747) {
			...
			if (player.getPet() != null) {
				...
				if (componentId == 12 || componentId == 21) {
					player.getDialogueManager().startDialogue("DismissD");
				}
		...
		if (player.getFamiliar() != null) {
			...
			if (componentId == 13 || componentId == 22) {
				player.getFamiliar().takeBob();
			}
			if (componentId == 14 || componentId == 23) {
				player.getFamiliar().renewFamiliar();
			}
			if (componentId == 19 || componentId == 10) {
