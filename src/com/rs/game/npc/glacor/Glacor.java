...
public class Glacor extends NPC {
	...
	public boolean attackable(Player player) {
		return this.target.getIndex() == player.getIndex() || player.getAttackedByDelay() > Utils.currentTimeMillis();
	}

	private void explode() {
		applyHit(new Hit(null, (int) (this.getHitpoints() * 0.90), HitLook.REGULAR_DAMAGE));
		...
		for (Entity entity : getPossibleTargets()) {
			...
			if (entity.withinDistance(this, 5)) {
				entity.applyHit(new Hit(null, (int) (target.getHitpoints() * 0.33), HitLook.REGULAR_DAMAGE));
			}
		}
	}
	...
	public void followTarget(Player player) {
		...
		if (distanceX < size && distanceX > -1 && distanceY < size && distanceY > -1 && !player.hasWalkSteps()
				&& !hasWalkSteps()) {
			resetWalkSteps();
		...
		if (!clipedProjectile(player, true)
			...
			calcFollow(player, getRun() ? 2 : 1, true, false);
	...
	private void processMinionSpawns() {
		enduring = new EnduringGlacyte(this, new WorldTile(this.getX() + 3, this.getY() + 1, this.getPlane()), target);
		sapping = new SappingGlacyte(this, new WorldTile(this.getX() + 2, this.getY() + 1, this.getPlane()), target);
		unstable = new UnstableGlacyte(this, new WorldTile(this.getX() + 1, this.getY() + 1, this.getPlane()), target);
	...
	public void processNPC() {
		...
		if (target != null) {
			...
			if (target instanceof Player) {
				...
				if (target.isDead() || player == null || player.hasFinished() || !player.isRunning()
		...
		if (target != null && !target.withinDistance(this, 100) && this.getCombat().underCombat()) {
		...
		if (this.getHitpoints() <= this.getMaxHitpoints() / 2 && target != null && !minionsSummoned
