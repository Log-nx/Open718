			...
			public void run() {
				if (target instanceof Player) {
					...
					if (!player.getPrayer().usingPrayer(
							...
							&& Utils.random(15) == 0) {
						player.sm("<col=01A9DB>The glacor fires a freezing attack!</col>");
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								if (target.matches(tile.get(0))
									...
									player.sm("You've been hit by the glacors freezing attack, it fires a punishing attack!");

								} else {
									player.sm("You break free from the ice!");
								}
								this.stop();
								tile.clear();
							}
