...
public class InstanceKalphiteQueen extends NPC {

	public InstanceKalphiteQueen(int id, WorldTile tile) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					reset();
					if (player != null) {
						if (player.getTimer() != 0 || player.getTimer() > 0 && id == 1160) {			
							new KalphiteQueen(1158, tile);	
						}
						} else {
							if (id == 1160) {
							new KalphiteQueen(1158, tile);
