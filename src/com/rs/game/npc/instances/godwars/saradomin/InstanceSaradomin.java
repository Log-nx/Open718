package com.rs.game.npc.instances.godwars.saradomin;
...
public class InstanceSaradomin extends NPC {
	
	int NPC = 6247;

	public InstanceSaradomin(int id, WorldTile tile) {
				...
				if (loop == 0) {
					...
					if (player != null) {
						if (player.getTimer() != 0 || player.getTimer() > 0 && id == NPC) {			
							new InstanceSaradomin(NPC, tile);
							}
						}
