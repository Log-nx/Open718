...
public class KalphiteKing extends NPC {
     ...
     * @param tile The WorldTile to spawn in.
     * @param mapAreaNameHash The Name of the Map Area hash.
     * @param canBeAttackFromOutOfArea if can be attacked from out of the area.
     * @param spawned if is a customly spawned NPC.
     */
    public KalphiteKing(int id, WorldTile tile) {
    ...
    public void sendDeath(final Entity source) {
		...
		if (player.getControllerManager().getController() instanceof KalphiteKingInstance && player.getTimer() != 0 || player.getTimer() > 0) {
			WorldTasksManager.schedule(new WorldTask() {

				@Override
				public void run() {
					new KalphiteKing(16697, getRespawnTile());
				}
