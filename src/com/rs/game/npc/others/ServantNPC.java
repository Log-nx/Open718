...
public class ServantNPC extends NPC {
	...
	public void makeFood(final Builds[] builds) {
		...
		String basicResponse = "I appologise, but I cannot serve " + (owner.getAppearence().isMale() ? "Sir" : "Madam") + " without";
	...
	public void requestType(int item, int quantity, final byte type) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				house.incrementPaymentStage();
				if (type == 0) {
					bank.withdrawItem(bank.getItemSlot(finalItem), completeQuantity);
				} else if (type == 1) {
