...
public class AgilityPenguinNPC extends NPC {
    ...
    public void giveReward(final Player player) {
    	...
    	if (player.getStatistics().getLapsRan() >= 250) {
			if (!player.hasItem(new Item(14936)))
				player.addItem(new Item(14936, 1));
		}
    	if (player.getStatistics().getLapsRan() >= 500) {
		...
		player.sm("<col=ff0000>The Agility Penguin gives you a reward before leaving.");
		player.lock(3);
