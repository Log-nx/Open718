...
public class RiverTrollNPC extends NPC {
    ...
    public void giveReward(final Player player) {
    	...
    	if (player.getStatistics().getFishCaught() >= 1000) {
			if (!player.getInventory().contains(24427))
				player.getInventory().addItem(24427, 1);
		}
    	if (player.getStatistics().getFishCaught() >= 2000) {
			if (!player.getInventory().contains(24428))
				player.getInventory().addItem(24428, 1);
		}
    	if (player.getStatistics().getFishCaught() >= 3000) {
			if (!player.getInventory().contains(24429))
				player.getInventory().addItem(24429, 1);
		}
    	if (player.getStatistics().getFishCaught() >= 4000) {
			if (!player.getInventory().contains(24430))
		...
		player.getPackets().sendGameMessage("<col=ff0000>The River Troll gives you a reward before leaving.");
		player.lock(3);
