...
public class RogueNPC extends NPC {
    ...
    public RogueNPC(WorldTile tile, Player target) {
    ...
    public void giveReward(final Player player) {
    	...
    	if (player.getStatistics().getTimesStolen() >= 1000) {
			if (!player.getInventory().contains(new Item(21483)) || !player.getBank().containsItem(21483, 1))
				player.getInventory().addItem(new Item(21483));
		}
    	if (player.getStatistics().getTimesStolen() >= 2000) {
			if (!player.getInventory().contains(new Item(21482)) || !player.getBank().containsItem(21482, 1))
				player.getInventory().addItem(new Item(21482));
		}
    	if (player.getStatistics().getTimesStolen() >= 3000) {
			if (!player.getInventory().contains(new Item(21481)) || !player.getBank().containsItem(21481, 1))
				player.getInventory().addItem(new Item(21481));
		}
    	if (player.getStatistics().getTimesStolen() >= 4000) {
			if (!player.getInventory().contains(new Item(21480)) || !player.getBank().containsItem(21480, 1))
				player.getInventory().addItem(new Item(21480));
		}
		stop = true;
		player.getMoneyPouch().sendDynamicInteraction(Utils.random(250000), false);
		setNextForceTalk(new ForceTalk("See you later, "+target.getDisplayName()+"!"));
		player.sm("<col=ff0000>The Rogue gives you a reward before leaving.");
		player.lock(3);
