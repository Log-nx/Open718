...
public class CoordsEvent {
	...
	public CoordsEvent(Object object, Runnable event) {
		this(object, event, false);
	}

	public CoordsEvent(Object object, Runnable event, boolean alternative) {
		this.object = object;
		this.event = event;
	...
	public boolean processEvent(final Player player) {
		...
		if (last != null && match(strategies, last) && player.hasWalkSteps())
			...
			for (int i = 0; i < strategies.length; i++) {
				...
				if (player.isCantWalk() || player.isFrozen())
					return false;
