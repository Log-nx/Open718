...
public class PrifddinasAgilityCourse {
	...
	private static void addExp(Player player, boolean last) {
		...
		if (last) {
			exp *= 10;
			player.getStatistics().addLapsRan();
	    	Agility.checkAgilityRandom(player);
	    	player.hefinLapReward = true;
	    	if (player.getStatistics().getHefinLaps() == 200) {
	    		player.getPackets().sendGameMessage("Congratulations! You've unlocked '<col=964F03>of the Hefin</col>' Loyalty Title.");
	    		player.getAchievementManager().unlockAchievement(Achievements.HEFIN_LAPS_200);
	    	}
    		player.getStatistics().addHefinLaps();
			player.getDialogueManager().startDialogue("LightCreature");
		    player.getPackets().sendGameMessage("You've completed the Agility course; laps ran: "
		    		+Colors.RED+Utils.getFormattedNumber(player.getStatistics().getLapsRan())+"</col>.", true);
		}
	...
	public static void jumpToOutcrop(final Player player, WorldObject object) {
		...
		if (player.getX() != 2266 && player.getY() != 3402) {
			player.getPackets().sendGameMessage("You can't reach that.", true);
			return;
		}
		player.lock();
		player.faceObject(object);
	...
	private static boolean canTraverse(Player player) {
		if (player.getSkills().getLevel(Skills.AGILITY) > 90
			...
			return true;
		player.getPackets().sendGameMessage("You need to have level 90 in Agility, Strength and Ranged to do this.");
		return false;
	}
