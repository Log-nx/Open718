...
import com.rs.game.player.actions.construction.HouseConstants.Builds;
import com.rs.game.player.actions.construction.HouseConstants.HObject;
import com.rs.game.player.actions.construction.HouseConstants.POHLocation;
import com.rs.game.player.actions.construction.HouseConstants.Roof;
import com.rs.game.player.actions.construction.HouseConstants.Room;
import com.rs.game.player.actions.construction.HouseConstants.Servant;
...
public class House implements Serializable {
	...
	public void openRoomCreationMenu(int roomX, int roomY, int plane) {
		...
		if (room != null) {
			...
			for (int index = 0; index < HouseConstants.Room.values().length - 2; index++) {
				...
				if (player.getSkills().getLevel(Skills.CONSTRUCTION) >= refRoom.getLevel() && player.getMoneyPouch().getCoinsAmount() >= (refRoom.getPrice())) {
					player.getPackets().sendIComponentText(402, index + (refRoom == HouseConstants.Room.DUNGEON_STAIRS 
					|| refRoom == HouseConstants.Room.DUNGEON_PIT ? 69 : refRoom == HouseConstants.Room.TREASURE_ROOM ? 70 : 68), 
	...
	public void createRoom(int slot) {
		...
		if (!player.hasStarted()) {
			...
			if (!(player.getMoneyPouch().getCoinsAmount() >= (room.getPrice()))) {
	...
	public void createRoom(RoomReference room) {
		...
		if (player.getMoneyPouch().getCoinsAmount() < (room.room.getPrice())) { //better double check if somehow u manage to drop money
			player.getPackets().sendGameMessage("You don't have enough coins to build this room.");
			return;
		}
		player.getMoneyPouch().sendDynamicInteraction(room.room.getPrice(), true);
		roomsR.add(room);
		refreshNumberOfRooms();
		refreshHouse();
	}
	...
	public void openBuildInterface(WorldObject object, final Builds build) {
		...
		for (int index = 0; index < build.getPieces().length; index++) {
			...
			player.getPackets().sendConfig(1485 + index, player.getSkills().getLevel(Skills.CONSTRUCTION) >= piece.getLevel() 
		...
		for (int i = 0; i < (interId == 396 ? 7 : 3); i++) {
			if (i >= build.getPieces().length) {
				...
				for (int i2 = 0; i2 < 4; i2++) {
					player.getPackets().sendIComponentText(interId, 15 + i2 + 5 * i, build.getPieces()[i].getRequirements().length <= i2 ? "" : build.getPieces()[i].getRequirements()[i2].getName()
													+ ": "
													+ build.getPieces()[i]
															.getRequirements()[i2]
															.getAmount());
					...
					for (WorldObject object : objects) {
						...
						if (remove) {
							if (challengeMode) {
								...
								//if (guardians != null)
								//	guardians.add(new Guard(oref.getPiece().getNPCId(), this, object)); //TODO
							} else {
	...
	public static void enterHouse(Player player, String displayname) {
		if (player.isLocked()) {
			...
			//like home tele which teleports you out when stepping off lodestone
			return;
		}
		...
		if (owner != player) {
			...
			//if (owner.getFriendsIgnores().getPmStatus() == FriendsIgnores.PM_STATUS_OFFLINE || (owner.getFriendsIgnores().getPmStatus() == FriendsIgnores.PM_STATUS_FRIENDSONLY && !owner.getFriendsIgnores().isFriend(player.getDisplayName()))) {
			//	player.getPackets().sendGameMessage("That player is offline, or has privacy mode enabled.");
			//	return;
			//} //TODO
		}
		if (!player.withinDistance(owner.getHouse().location.getTile(), 16)) {
			player.getPackets().sendGameMessage("This house is at " + Utils.formatPlayerNameForDisplay(owner.getHouse().location.name()) + ".");
			return;
		}
			...
			for (int y = 0; y < 8; y++) {
				...
				if (objects != null) {
					for (WorldObject object : objects) {
						...
						if (defs.containsOption(0, "Build")) {
							WorldObject objectR = new WorldObject(object);
	...
	public void createHouse(final boolean tp) {
		if (player.getHouse().getLocation() == null) {
			...
			player.getPackets().sendGameMessage("Default house Location set to: Edgeville.");
		}
		loaded = false;
						...
						for (int y = 0; y < 8; y++) {
							...
							if (objects != null) {
								skip: for (WorldObject object : objects) {
									...
									if (object.getDefinitions().containsOption(0, "Build") || object.getDefinitions().name.equals("Habitat space")) {
										if (isDoor(object)) {
	...
	public static boolean createRoom(Player player, int interfaceId, int componentId, int slotId) {
		if (interfaceId == 402) {
