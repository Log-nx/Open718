package com.rs.game.player.actions.cooking;
...
public class Cooking extends Action {
	 ...
	 * Cooked Meat
	 */
	RAW_MEAT(new Item(2132, 1), 1, 20, 30, new Item(2146, 1), new Item(2142, 1), false, false),
	RAW_RAT_MEAT(new Item(2134, 1), 1, 20, 30, new Item(2146, 1), new Item(2142, 1), false, false),
	RAW_BEAR_MEAT(new Item(2136, 1), 1, 20, 30, new Item(2146, 1), new Item(2142, 1), false, false),
    ...
    public static double increasedExperience(Player player, double totalXp) {
    	...
    	if (player.getPerkManager().hasPerk(PlayerPerks.CERTIFIED_CHEF)) {
			totalXp *= 1.25;
		}
    	if (player.getTemporaryAttributtes().get(Key.USING_PORTABLE_RANGE) != null) {
    		totalXp *= 1.21;
    	}
    	return totalXp;
    }
    ...
    private boolean isBurned(Cookables cook, Player player) {
		...
		if (player.getPerkManager().hasPerk(PlayerPerks.NO_BURNING) || player.getControllerManager().getController() instanceof DiccusTutorial && player.getStatistics().tutorialStage == 7) {
    ...
    public boolean process(Player player) {
		...
		if (player.getSkills().getLevel(Skills.COOKING) < cook.getLvl()) {
		    player.getPackets().sendGameMessage("You need a Cooking level of " + cook.getLvl() + " to cook this.");
		    return false;
		}
		if (Utils.random(350) == 0) {
			new CookNPC(player, player);
			player.getPackets().sendGameMessage("<col=ff0000>A Mysterious Cook appears from the "+object.getDefinitions().name+".");
		}
		return true;
    }
    ...
    public int processWithDelay(Player player) {
		amount--;
		if (player.getPerkManager().hasPerk(PlayerPerks.SAMURAI_COOKING)) {
			player.setNextAnimation(new Animation(17314));
			player.setNextGraphics(new Graphics(3306));
		} 
		else if (player.getPerkManager().hasPerk(PlayerPerks.ARCANE_COOKING)) {
		...
		if (Utils.random(2) == 0 && player.getSkills().getLevel(Skills.COOKING) 
					>= cook.getBurningLvl() ? false : isBurned(cook, player)) {
			...
			if (cook.getBurntId() == Cookables.RAW_SHRIMP.getBurntId() && player.getControllerManager().getController() instanceof DiccusTutorial && player.hasntCookedShrimpYet == true) {
				Dialogue.closeNoContinueDialogue(player);
				player.getDialogueManager().startDialogue("SimpleNPCMessage", 943, "Oh, you burnt your shrimp? Don't worry...that happens to everybody the first time. As you get more experience in Cooking, you'll burn food less often. Let's try again. First, fish some shrimps. Then use them on a fire. If your fire has gone out, chop some logs to make a new fire.");
			}
		} else {
			if (cook.getRawItem().getId() == Cookables.RAW_SHRIMP.getRawItem().getId() && player.getControllerManager().getController() instanceof DiccusTutorial && player.hasntCookedShrimpYet == true) {
				Dialogue.closeNoContinueDialogue(player);
				player.hasntCookedShrimpYet = false;
				player.getStatistics().tutorialStage = 6;
				player.getDialogueManager().startDialogue("SimpleNPCMessage", 943, "Oh, well done! Now you have some food to eat whenever you feel poorly. I've taught you all I can about Woodcutting, Firemaking, and Fishing. Open the gate, follow the path to the next area and talk to the master chef! He'll teach you more about Cooking!");
			}
			if (cook.getRawItem().getId() == Cookables.BREAD.getRawItem().getId() && player.getControllerManager().getController() instanceof DiccusTutorial) {
				player.getStatistics().tutorialStage = 8;
				player.getDialogueManager().startDialogue("MasterChef", 3);
			}
		    player.getInventory().deleteItem(item.getId(), 1);
		    ...
		    player.getStatistics().addFoodCooked();
		    player.getPackets().sendGameMessage("You successfully cook the "
				    + cook.getProduct().getDefinitions().getName().toLowerCase() + "; "
				    		+ "items cooked: "+Colors.RED+Utils.getFormattedNumber(player.getStatistics().getFoodCooked())+"</col>.", true);
			handleSkillTasks(player);
			player.getCompCapeManager().increaseRequirement(Requirement.COOK_FISH, 1);
		}
		if (amount > 0) {
		    player.getPackets().sendGameMessage("You attempt to cook the " 
		    		+ cook.getProduct().getDefinitions().getName().toLowerCase() + ".", true);
		    return player.getPerkManager().hasPerk(PlayerPerks.CERTIFIED_CHEF) ? 2 : 3;
		}
		return -1;
    }
    ...
    public void stop(final Player player) {
    	setActionDelay(player, player.getPerkManager().hasPerk(PlayerPerks.CERTIFIED_CHEF) ? 2 : 3);
    }
    ...
    public static double chefsSuit(Player player) {
    	...
    	if (player.getEquipment().getHatId() == 25180
    			&& player.getEquipment().getChestId() == 25181
    			&& player.getEquipment().getLegsId() == 25182
    			&& player.getEquipment().getBootsId() == 25183
	...
	private void handleSkillTasks( Player player ) {
		if (cook == Cookables.RAW_ANCHOVIES) {
			player.getSkillTasks().decreaseTask( SkillTasks.CANCHOVIES1);
			player.getSkillTasks().decreaseTask(SkillTasks.CANCHOVIES2);
		} else if (cook == Cookables.RAW_HERRING) {
			player.getSkillTasks().decreaseTask(SkillTasks.CHERRING1);
			player.getSkillTasks().decreaseTask(SkillTasks.CHERRING2);
		} else if (cook == Cookables.RAW_LOBSTER) {
			player.getSkillTasks().decreaseTask(SkillTasks.CLOBSTER1);
			player.getSkillTasks().decreaseTask(SkillTasks.CLOBSTER2);
		} else if (cook == Cookables.RAW_ROCKTAIL) {
			player.getSkillTasks().decreaseTask(SkillTasks.CROCKTAIL1);
			player.getSkillTasks().decreaseTask(SkillTasks.CROCKTAIL2);
		} else if (cook == Cookables.RAW_SALMON) {
			player.getSkillTasks().decreaseTask(SkillTasks.CSALMON1);
			player.getSkillTasks().decreaseTask(SkillTasks.CSALMON2);
		} else if (cook == Cookables.RAW_SHARK) {
			player.getSkillTasks().decreaseTask(SkillTasks.CSHARK1);
			player.getSkillTasks().decreaseTask(SkillTasks.CSHARK2);
			player.getSkillTasks().decreaseTask(SkillTasks.CSHARK3);
		} else if (cook == Cookables.RAW_SHRIMP) {
			player.getSkillTasks().decreaseTask(SkillTasks.CSHRIMP1);
			player.getSkillTasks().decreaseTask(SkillTasks.CSHRIMP2);
		} else if (cook == Cookables.RAW_SWORDFISH) {
			player.getSkillTasks().decreaseTask(SkillTasks.CSWORD1);
			player.getSkillTasks().decreaseTask(SkillTasks.CSWORD2);
		} else if (cook == Cookables.RAW_TROUT) {
			player.getSkillTasks().decreaseTask(SkillTasks.CTROUT1);
			player.getSkillTasks().decreaseTask(SkillTasks.CTROUT2);
		} else if (cook == Cookables.RAW_TUNA) {
			player.getSkillTasks().decreaseTask(SkillTasks.CTUNA1);
			player.getSkillTasks().decreaseTask(SkillTasks.CTUNA2);
		}
	}
