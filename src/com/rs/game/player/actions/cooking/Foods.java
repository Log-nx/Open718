...
public class Foods {
	...
	public static boolean eat(final Player player, Item item, int slot) {
		...
		if	(player.isUnderCombat() && player.rs3Combat){
			player.setNextAnimation(new Animation(18002));
		} else {
			player.setNextAnimation(new Animation(18001));
		}
		...
		player.applyHit(new Hit(player, food.getHeal() * 10, HitLook.HEALED_DAMAGE));
