    ...
    public int processWithDelay(Player player) {
    	if (checkAll(player)) {
			...
			if (Utils.random(5) != 0) {
				...
				player.getPackets().sendGameMessage("You successfully dry the meat into sinew.", true);
			} else {
			    player.getInventory().addItem(2146, 1);
				player.getSkills().addXp(Skills.COOKING, 1);
				player.getPackets().sendGameMessage("You failed drying the meat into sinew.", true);
			}
		}
		return 0;
    }
