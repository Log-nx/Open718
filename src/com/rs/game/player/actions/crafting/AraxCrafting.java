...
public class AraxCrafting {
	...
	public static void handleSpiderLeg(Player player) {
		if (!player.getInventory().containsItem(31718, 1) || !player.getInventory().containsItem(31719, 1)
			...
			player.getPackets().sendGameMessage("You'll need all 3 Spider leg pieces in order to do this.");
			return;
		}
		if (player.getSkills().getLevelForXp(Skills.CRAFTING) < 30) {
			player.getPackets().sendGameMessage("You'll need a Crafting level of at least 30 in order to do this.");
			return;
		}
		...
		player.getPackets().sendGameMessage("You've successfully made a full, gross.. spider leg!");
	}
	...
	public static boolean canCraftWeapon(Player player, int bodyID) {
		if (!player.getInventory().containsOneItem(31721)) {
			player.getPackets().sendGameMessage("You do not have a Spider leg to work with.");
			return false;
		}
		if (!player.getInventory().containsOneItem(bodyID)) {
			player.getPackets().sendGameMessage("You do not have an Araxxi's body piece to work with.");
			return false;
		}
		if (player.getSkills().getLevel(Skills.CRAFTING) < 90) {
			player.getPackets().sendGameMessage("You'll need a Crafting level of at least 90 in order to do this.");
			return false;
		}
		return true;
	}
