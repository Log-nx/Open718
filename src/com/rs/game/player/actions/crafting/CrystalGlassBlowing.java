...
public class CrystalGlassBlowing extends Action {
	...
	private void blow(Player player) {
		...
		player.getPackets().sendGameMessage("You form a figure out of the glass; " + "items crafted: " + Colors.RED
				+ Utils.getFormattedNumber(player.getStatistics().getItemsMade()) + "</col>.", true);
	}
	...
	public boolean process(Player player) {
		if (!player.getInventory().containsItem(new Item(32845, 1))
			...
			player.getPackets().sendGameMessage("You have no more glass to work with.");
			return false;
		}
		return true;
	}
	...
	public boolean start(Player player) {
		if (player.getSkills().getLevel(Skills.CRAFTING) < 89) {
			player.getPackets().sendGameMessage("You need a Crafting level of 89 to make this.");
			return false;
		}
		if (!player.getInventory().containsItem(new Item(32845, 1))
				&& !player.getInventory().containsItem(new Item(23193, 1))) {
		...
		if (!player.getInventory().containsOneItem(1785)) {
			player.getPackets().sendGameMessage("You do not have a glassblowing pipe to work the glass with.");
			return false;
		}
		return true;
	}
