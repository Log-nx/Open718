...
public class Glassblowing {
	...
	private static void blow(Player player, GlassblowingItem item) {
		if (player.getSkills().getLevel(Skills.CRAFTING) < item.getSkillRequirement()) {
			player.getPackets().sendGameMessage("You need a Crafting level of " + item.getSkillRequirement() + " to make this.");
			return;
		}
		...
		player.getPackets().sendGameMessage("You form a figure out of the glass; "
