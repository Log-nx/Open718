...
/*
 * Used to handle Leather crafting.
...
public class LeatherCrafting extends Action {
    ...
    public static boolean handleItemOnItem(Player player, Item itemUsed, Item usedWith) {
		for (int i = 0; i < LEATHER.length; i++) {
		    if (itemUsed.getId() == LEATHER[i] || usedWith.getId() == LEATHER[i]) {
				...
				if (index == -1)
				    return true;
				if (itemUsed.getId() == 6289 || usedWith.getId() == 6289 || itemUsed.getId() == 1741 || usedWith.getId() == 1741)
    ...
    public static final int PRODUCTS[][] = { { 1169, 1129, 1095, 1059, 1061, }, { 1065, 1099, 1135 },
	    ...
	    { 6326, 6322, 6324, 6328, 6330 }, { 24376, 24379, 24382 } };
    ...
    private boolean checkAll(Player player) {
		if (player.getInterfaceManager().containsScreenInter() 
		    ...
		    player.sm("Please finish what you're doing before doing this action.");
		    return false;
		}
		if (data.getRequiredLevel() > player.getSkills().getLevel(
			Skills.CRAFTING)) {
		    player.sm("You need a crafting level of " + data.getRequiredLevel()
				    + " to craft this hide.");
		    return false;
		}
		if (player.getInventory().getItems().getNumberOf(data.getLeatherId()) < data.getLeatherAmount()) {
		    player.sm("You don't have enough amount of hides in your inventory.");
		    return false;
		}
		if (!player.getInventory().getItems().containsOne(THREAD) && !player.getPerkManager().hasPerk(PlayerPerks.DELICATE_CRAFTSMAN)) {
		    player.sm("You need some thread to do this.");
		    return false;
		}
		if (!player.getInventory().containsOneItem(NEEDLE.getId())) {
		    player.sm("You need a needle to craft leather.");
		    return false;
		}
		if (!player.getInventory().containsOneItem(data.getLeatherId())) {
		    player.sm("You've ran out of " + 
    ...
    public int processWithDelay(Player player) {
		...
		player.getSkills().addXp(Skills.CRAFTING, data.getExperience() * (player.getPerkManager().hasPerk(PlayerPerks.DELICATE_CRAFTSMAN) ? 1.25 : 1));
		...
		quantity--;
		if (!player.getPerkManager().hasPerk(PlayerPerks.DELICATE_CRAFTSMAN)) {
			removeThread--;
			if (removeThread == 0) {
			    removeThread = Utils.random(2, 6);
			    player.getInventory().removeItems(THREAD);
			    player.getPackets().sendGameMessage("You use up a reel of your thread.", true);
			}
		}
		if (quantity <= 0)
		    return -1;
	...
	public static LeatherData getLeather(Player player) {
		for (LeatherData leather : LeatherData.values()) {
			if (player.getInventory().containsOneItem(leather.getLeatherId()))
				return leather;
		}
		return null;
	}
