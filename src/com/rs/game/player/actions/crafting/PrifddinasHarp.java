...
public class PrifddinasHarp extends Action {
    ...
    private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.CRAFTING) < 75) {
			player.getPackets().sendGameMessage("You need a Crafting level of at least 75 to do this activity.");
		    return false;
		}
		return true;
    }
    ...
    public boolean start(Player player) {
		...
		if (!player.getInventory().addItem(new Item(32622, 1))) {
			player.getPackets().sendGameMessage("Not enough inventory space for Harmonic dust.", true);
			return false;
		}
