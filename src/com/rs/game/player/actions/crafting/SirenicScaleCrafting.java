...
public class SirenicScaleCrafting extends Action {
    ...
    public boolean process(Player player) {
		...
		if (player.getSkills().getLevel(Skills.CRAFTING) < scale.getLevelRequired()) {
			player.sm("You need a Crafting level of at least " + scale.getLevelRequired() + " to create a " + scale.getProduceEnergy().getDefinitions().getName());
		    return false;
		}
		...
		if (!player.getInventory().containsOneItem(1733)) {
		    player.sm("You need a needle to craft Sirenic scale.");
		    return false;
		}
		return true;
    }
    ...
    public int processWithDelay(Player player) {
		...
		player.getPackets().sendGameMessage("You make a " + scale.getProduceEnergy().getDefinitions().getName().toLowerCase() + "; "
