...
public class TectonicEnergyCrafting extends Action {
	...
	public enum Tectonic {
		...
		TECTONIC_MASK(91, 500, new Item[] { new Item(28627, 14) }, new Item(28608)),
		TECTONIC_ROBE_TOP(93, 1500, new Item[] {new Item(28627, 42) }, new Item(28611)),
		TECTONIC_ROBE_BOTTOM(92, 1000, new Item[] {new Item(28627, 28) }, new Item(28614));
    ...
    public boolean process(Player player) {
		...
		if (player.getSkills().getLevel(Skills.RUNECRAFTING) < scale.getLevelRequired()) {
			player.getPackets().sendGameMessage("You need a Runecrafting level of at least " + scale.getLevelRequired() + " to create a " + scale.getProduceEnergy().getDefinitions().getName());
		    return false;
		}
    	int amount = scale.getItemsRequired()[0].getAmount();
		if (!player.getInventory().containsItem(scale.getItemsRequired()[0].getId(), amount)) {
			player.getPackets().sendGameMessage("You need "+(amount > 1 ? "x"+amount+" of" : "a")+" " + scale.getItemsRequired()[0].getDefinitions().getName() + " to create " + scale.getProduceEnergy().getDefinitions().getName() + ".");
		    return false;
		}
		if (scale.getItemsRequired().length > 0) {
		    ...
		    if (!player.getInventory().containsItem(scale.getItemsRequired()[1].getId(), amount)) {
				player.getPackets().sendGameMessage("You need "+(amount > 1 ? "x"+amount+" of" : "a")+" " + scale.getItemsRequired()[1].getDefinitions().getName() + " to create " + scale.getProduceEnergy().getDefinitions().getName() + ".");
				return false;
		    }
		}
		return true;
