...
public class DivineObject {
    ...
    private static boolean canPlaceLocation(Player owner, int plane, int x, int y) {
    	if (!World.canMoveNPC(plane, x, y, 2)
				...
				|| owner.getControllerManager().getController() != null
				|| World.containsObjectWithId(new WorldTile(x, y, plane), 9542))
			return false;
    	return true;
    }
    ...
    public static void placeDivine(final Player owner, final int item, final int fobject, final int lobject, int lvl, int skillId) {
    	if (owner.getSkills().getLevel(skillId) < lvl) {
    		owner.getPackets().sendGameMessage("You need a "+owner.getSkills().getSkillName(skillId)+" level of "+lvl+" to place this Divine location.");
    		return;
    	}
    	...
    	if (!canPlaceLocation(owner, tile.getPlane(), tile.getX(), tile.getY())) {
        	...
        	if (!canPlaceLocation(owner, tile.getPlane(), tile.getX(), tile.getY())) {
            	...
            	if (!canPlaceLocation(owner, tile.getPlane(), tile.getX(), tile.getY())) {
    	        	...
    	        	if (!canPlaceLocation(owner, tile.getPlane(), tile.getX(), tile.getY())) {
    					owner.getPackets().sendGameMessage("You cannot place a Divine location here; try moving around.");
    					return;
    				}
    			}
    		}
    ...
    private static void resetDivineCreation(Player player) {
    	...
    	if (player.created)
    		player.getPackets().sendGameMessage(Colors.DARK_RED+"Your Divination Divine location creation limit has been reset.");
    ...
    public static void resetGatherLimit(Player player) {
    	...
    	if (player.gathered > 0)
    		player.getPackets().sendGameMessage(Colors.DARK_RED+"Your Divination Divine location gather limit has been reset.");
    	player.gathered = 0;
    }
	...
	public static int checkPercentage(Player player) {
		...
		int maxCharges = player.getSkills().getLevelForXp(Skills.DIVINATION) + 16;
    ...
    public static boolean canHarvest(Player player) {
		if (checkPercentage(player) >= 100) {
			player.getPackets().sendGameMessage(Colors.DARK_RED+"You have already gathered your limit from Divine locations today.");
    ...
    public static void handleHarvest(Player player) {
		...
		if (checkPercentage(player) == 25)
			player.getPackets().sendGameMessage(Colors.DARK_RED+"You have used 25% of your daily gathering limit for Divine locations.", true);
		if (checkPercentage(player) == 50)
			player.getPackets().sendGameMessage(Colors.DARK_RED+"You have used 50% of your daily gathering limit for Divine locations.");
		if (checkPercentage(player) == 75)
			player.getPackets().sendGameMessage(Colors.DARK_RED+"You have used 75% of your daily gathering limit for Divine locations.", true);
    }
}