...
public class DivineFishing extends Action {
	...
	private void addFish(Player player) {
		...
		player.getStatistics().addFishCaught(false);
		player.getPackets().sendGameMessage("You catch a " + fishName + "; fish caught: "
				+ Colors.RED+Utils.getFormattedNumber(player.getStatistics().getFishCaught())+"</col>.", true);
		...
		if (roll >= 75 && owner != null && player != owner && DivineObject.checkPercentage(owner) < 100) {
			...
			owner.getPackets().sendGameMessage(player.getDisplayName()+" caught a "+fishName+" for you.", true);
		}
	}
