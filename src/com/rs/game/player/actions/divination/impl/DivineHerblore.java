...
public class DivineHerblore extends Action {
	...
	private void addHerb(Player player) {
		...
		player.getStatistics().addProduceGathered();
		player.getPackets().sendGameMessage("You harvest a " + herbName + "; produce gathered: "
				+ Colors.RED+Utils.getFormattedNumber(player.getStatistics().getProduceGathered())+"</col>.", true);
		...
		if (roll >= 75 && owner != null && player != owner && DivineObject.checkPercentage(owner) < 100) {
			...
			owner.getPackets().sendGameMessage(player.getDisplayName()+" harvested a "+herbName+" for you.", true);
		}
	}
