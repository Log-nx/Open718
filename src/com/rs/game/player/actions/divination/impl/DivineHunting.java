...
public class DivineHunting extends Action {
	...
	private void addItem(Player player) {
		...
		player.getPackets().sendGameMessage(getMessage(player, hItem), true);
		...
		if (roll >= 75 && owner != null && player != owner && DivineObject.checkPercentage(owner) < 100) {
			...
			owner.getPackets().sendGameMessage(player.getDisplayName()+" caught a "+name+" for you.", true);
		}
	}
	
	private String getMessage(Player player, Item hunt) {
		return "You manage to catch a " + hunt.getDefinitions().getName().toLowerCase() + "; "
				+ "creatures caught: "+Colors.RED+Utils.getFormattedNumber(player.getStatistics().getCreaturesCaught())+"</col>.";
	}

	private boolean checkAll(Player player) {
		Player owner = spott.getOwner();
		if (player != owner && player.isIronman()) {
		...
		if (player.getSkills().getLevel(Skills.HUNTER) < spot.getRewards()[itemId].getLevel()) {
			player.setNextAnimation(new Animation(-1));
			player.getPackets().sendGameMessage("You need a Hunter level of at least "+spot.getRewards()[itemId].getLevel()+" to use this Divine location.");
			return false;
		}
