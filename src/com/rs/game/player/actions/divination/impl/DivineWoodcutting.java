...
public final class DivineWoodcutting extends Action {
    ...
    private void addLog(Player player) {
		...
		player.getPackets().sendGameMessage("You get some " + hItem.getName().toLowerCase()
				+ "; total chopped: "+ Colors.RED+Utils.getFormattedNumber(player.getStatistics().getLogsChopped())+"</col>.", true);
		...
		if (roll >= 75 && owner != null && player != owner && DivineObject.checkPercentage(owner) < 100) {
			...
			owner.getPackets().sendGameMessage(player.getDisplayName()+" chopped some "+logName+" for you.", true);
		}
    }
