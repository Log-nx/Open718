...
public class FarmingManager implements Serializable {
	...
	private static final int ALLOTMENT = 0, TREES = 1, HOPS = 2, FLOWERS = 3, FRUIT_TREES = 4, BUSHES = 5, HERBS = 6,
			COMPOST = 7, MUSHROOMS = 8, BELLADONNA = 9, PILLAR = 10;
	...
	public enum ProductInfo implements Serializable {
		/**
		 * Allotments
		 */
		...
		Fellstalk(21621, 91, 21626, 6, 225, 315.6, 20, HERBS), Wergali(14870, 46, 213, 8, 52.8, 52.8, 20, HERBS),
		 ...
		 * Trees
		 */
		Oak(5370, 15, 6043, 1, 467.3, 14, 40, TREES), Willow(5371, 30, 6045, 6, 1456.5, 25, 40, 6, TREES),
		Maple(5372, 45, 6047, 17, 3403.4, 45, 40, 8, TREES), Yew(5373, 60, 6049, 26, 7069.9, 81, 40, 10, TREES),
		Magic(5374, 75, 6051, 41, 13768.3, 145.5, 40, 12, TREES),
		ELDER(32294, 90, 6051, 65, 23473.5, 180.5, 40, 14, TREES),
		
		/*
		 * int seedId, int level, int productId, int configIndex, double
		 * plantingExperience, double experience, int cycleTime, int maxStage, int
		 * stageSkip, int type
		 */
	...
	public static enum SpotInfo {
		Talvery_Tree(8388, TREES), Falador_Garden_Tree(8389, TREES), Varrock_Tree(8390, TREES),
		Lumbridge_Tree(8391, TREES), Gnome_Tree(19147, TREES), Prif_Tree(93288, TREES),
		Port_Sarim_Spirit_Tree(8338, TREES),

		Gnome_Strong_Fruit_Tree(7962, FRUIT_TREES), Gnome_Fruit_Tree(7963, FRUIT_TREES),
		Brimhaven_Fruit_Tree(7964, FRUIT_TREES), Catherby_Fruit_Tree(7965, FRUIT_TREES),
		Lletya_Fruit_Tree(28919, FRUIT_TREES), Priff_Fruit_Tree(94323, FRUIT_TREES),

		Falador_Allotment_North(8550, ALLOTMENT), Falador_Allotment_South(8551, ALLOTMENT),
		Catherby_Allotment_North(8552, ALLOTMENT), Catherby_Allotment_South(8553, ALLOTMENT),
		Ardougne_Allotment_North(8554, ALLOTMENT), Ardougne_Allotment_South(8555, ALLOTMENT),
		Canfis_Allotment_North(8556, ALLOTMENT), Canfis_Allotment_South(8557, ALLOTMENT),

		Yannile_Hops_Patch(8173, HOPS), Talvery_Hops_Patch(8174, HOPS), Lumbridge_Hops_Patch(8175, HOPS),
		McGrubor_Hops_Patch(8176, HOPS),

		Falador_Flower(7847, FLOWERS), Catherby_Flower(7848, FLOWERS), Ardougne_Flower(7849, FLOWERS),
		Canfis_Flower(7850, FLOWERS),

		Champions_Bush(7577, BUSHES), Rimmington_Bush(7578, BUSHES), Etceteria_Bush(7579, BUSHES),
		South_Arddougne_Bush(7580, BUSHES), Priff_Bush(93285, BUSHES),

		Falador_Herb_Patch(8150, HERBS), Catherby_Herb_Patch(8151, HERBS), Ardougne_Herb_Patch(8152, HERBS),
		Canfis_Herb_Patch(8153, HERBS), Priff_Herb_Patch(93287, HERBS),

		Falador_Compost_Bin(7836, COMPOST), Catherby_Bin(7837, COMPOST), Port_Phasymatis_Bin(7838, COMPOST),
		Ardougn_Bin(7839, COMPOST), Taverly_Bin(66577, COMPOST),

		Priff_Pillar(94326, PILLAR), Priff_Pillar2(94325, PILLAR), Priff_Pillar3(94324, PILLAR),
		...
		if (!spot.isCleared()) {
			...
			if (item != null) {
				...
				case 1:
					if (info.type == TREES) {
						...
						if (spot.reachedMaxStage() && !spot.hasChecked()) {
							...
							player.getPackets().sendGameMessage("The weeds haven't finished decomposing yet.");
						else
				...
				case 2:// inspect... usless tbh
					player.getPackets().sendGameMessage("There's something growing in this patch..");
					break;
				case 3:// clear & guide
					if (spot.isDiseased() || spot.reachedMaxStage() || spot.isDead()) {
						clearFarmingPatch(spot);
					} else if (spot.productInfo.type == FRUIT_TREES) {
						if (spot.reachedMaxStage())
							return;
	...
	private void startRakeAction(final FarmingSpot spot) {
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (!player.getInventory().containsItemToolBelt(5341)) {
					player.getPackets().sendGameMessage("You need a rake to do this.");
					return false;
				}
				return true;
			}
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean process(Player player) {
				if (spot.harvestAmount > 0)
					...
					player.getPackets().sendGameMessage("You harvest the produce in the " + patchName
							+ " patch; produce harvested: "+Colors.RED
								+Utils.getFormattedNumber(player.getStatistics().getProduceGathered())+"</col>.", true);
					player.setNextAnimation(new Animation(-1));
					spot.setIdle();
					return false;
				}
			...
			@Override
			public int processWithDelay(Player player) {
				if (player.getEquipment().hasFarmingCape()) {
					player.getGamePointManager().addGamePointItem(spot.productInfo.productId, spot.harvestAmount);
					player.getStatistics().produceGathered += spot.harvestAmount;
					if (spot.productInfo.type == TREES || spot.productInfo.type == FRUIT_TREES) {
						player.getCompCapeManager().increaseRequirement(Requirement.FARMING, 1);
					}
					player.getPackets().sendGameMessage(Color.GREEN, "Your farming cape has allowed you to finish this haverst instantly.");
					player.getSkills().addXp(Skills.FARMING, spot.productInfo.experience * spot.harvestAmount);
					spot.harvestAmount = 0;
					return 0;
				}
				if (player.getEquipment().containsOneItem(ItemIdentifiers.MAGIC_SECATEURS) && Utils.random(5) == 1)
					player.getPackets().sendGameMessage("Your magic secateurs prevent your plant from losing a life.");
				else
					spot.harvestAmount--;
				if (spot.productInfo.type == TREES || spot.productInfo.type == FRUIT_TREES) {
					player.getCompCapeManager().increaseRequirement(Requirement.FARMING, 1);
				...
				player.getGamePointManager().addGamePointItem(spot.productInfo.productId, 1);
				int amount = 1;
				if (spot.productInfo.type == ALLOTMENT || spot.productInfo.type == HOPS || spot.productInfo.type == HERBS && player.getEquipment().containsOneItem(ItemIdentifiers.MAGIC_SECATEURS) && Utils.random(10) == 1) {
					player.getPackets().sendGameMessage("Your magic secateurs gain you an extra produce.");
					amount++;
				}
				return 2;
			}
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean process(Player player) {
				if (spot.harvestAmount > 0)
					...
					player.getPackets().sendGameMessage("You pick all of the " + (spot.productInfo.type == FRUIT_TREES ? "fruits" : "berries") + " from the " + getPatchName(spot.productInfo.type) + " patch; produce harvested: " + Colors.RED + Utils.getFormattedNumber(player.getStatistics().produceGathered)+"</col>.", true);
					player.setNextAnimation(new Animation(-1));
					return false;
				}
			}
			...
			public int processWithDelay(Player player) {
				...
				player.setNextAnimation(getHarvestAnimation(spot.productInfo.type));
				player.getStatistics().getProduceGathered();
				player.getSkills().addXp(Skills.FARMING,spot.productInfo.experience);
	...
	public void clearFarmingPatch(final FarmingSpot spot) {
		...
		player.getActionManager().setAction(new Action() {
			...
			@Override
			public boolean start(Player player) {
				if (!player.getInventory().containsOneItem(952) || !player.getInventory().containsItemToolBelt(952)) {
					player.getPackets().sendGameMessage("You need a spade to do this.");
					return false;
				}
	...
	public boolean startFarmingCycle(FarmingSpot spot, Item item) {
		...
		if (!player.getInventory().containsOneItem(5343) || !player.getInventory().containsItemToolBelt(5343)) {
			player.getPackets().sendGameMessage("You need a seed dibber to do this.");
			return false;
		}
		if (!player.getInventory().containsOneItem(5325) || !player.getInventory().containsItemToolBelt(5325)) {
			player.getPackets().sendGameMessage("You need a gardening trowel to do this.");
			return false;
		}
		...
		if (isTree) {
			if (player.getInventory().hasFreeSlots())
				...
				World.addGroundItem(new Item(5350), player, player, true, 60);
		}
	...
	private void collectTreeProducts(final FarmingSpot spot, final TreeDefinitions definitions) {
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (!checkAll(player))
					return false;
				player.getPackets().sendGameMessage("You swing your hatchet at the tree...");
			...
			private boolean hasWoodcuttingLevel(Player player) {
				if (definitions.getLevel() > player.getSkills().getLevel(8)) {
					player.getPackets().sendGameMessage("You need a woodcutting level of "
			...
			private void addLog(Player player) {
			    ...
			    if (definitions == TreeDefinitions.IVY)
					...
					player.getPackets().sendGameMessage("You get some " + logName
							+ "; total chopped: "+ Utils.getFormattedNumber(player.getStatistics().getLogsChopped())+".", true);
					...
					if (infAdze == 1 && player.getEquipment().getWeaponId() == 13661
						...
						player.getPackets().sendGameMessage("The adze's heat instantly incinerates the " + logName + "; "
								+ "logs burned: "+Utils.getFormattedNumber(player.getStatistics().getLogsBurned())+".", true);
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(Player player) {
				if (item == null
					...
					player.getPackets().sendGameMessage("You cannot add that to the compost bin.");
					return false;
				}
				return true;
			}
	...
	private void openGuide() {
		player.getPackets().sendGameMessage("Google 'RS Wiki Farming' if you need help with training Farming.");
	}
	...
	public class FarmingSpot implements Serializable {
		...
		public void renewCycle() {
			...
			if (productInfo != null)
				cycleTime += GlobalEvents.isActiveEvent(Event.FASTER_SKILLING) ? constant * productInfo.cycleTime / 2 :  constant * productInfo.cycleTime;
			else
		...
		private void checkDisease() {
			if (canBeDiseased()) {
				...
				if (getCompost())
					...
					baseValue += 20;
				if (player.getPerkManager().hasPerk(PlayerPerks.FARMING_MASTER))
					baseValue += 25;
		...
		public void refresh() {
			...
			player.getPackets().sendConfigByFile(spotInfo.configFileId, value, true);
		}
	...
	public boolean checkWaterCan(Player player) {
		if (!player.getInventory().containsOneItem(5333, 5334, 5335, 5336, 5337, 5338, 5339, 5340)) {
			player.getPackets().sendGameMessage("You will need a watering can with water in it to do this!");
			return true;
		}
