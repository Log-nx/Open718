...
public static double increasedExperience(Player player, double totalXp) {
	...
	if (player.getEquipment().getRingId() == 13659)
		totalXp *= 1.02;
	if (player.getEquipment().getGlovesId() == 13660 &&
			player.getEquipment().getRingId() == 13659)
		totalXp *= 1.01;
...
public boolean checkAll(Player player) {
	...
	if (!player.getInventory().containsItem(590, 1) && !player.getInventory().containsItemToolBelt(590)) {
	...
	if (!World.canMoveNPC(player.getPlane(), player.getX(), player.getY(), 1) // cliped
			...
			|| player.getRegionId() == 9770 || player.getRegionId() == 10026) {
	    player.getPackets().sendGameMessage("You can't light a fire here; find a different area.");
	    return false;
	}
	return true;
}
...
public int processWithDelay(final Player player) {
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		public void run() {
			...
			World.spawnTempGroundObject(new WorldObject(fire.getFireId(), 10, 0, tile.getX(), tile.getY(), tile.getPlane()), 592, fire.getLife(), true);
			if (fire == Fire.NORMAL) {
				if (player.getControllerManager().getController() instanceof DiccusTutorial && player.getStatistics().tutorialStage == 3) {
					player.getStatistics().tutorialStage = 4;
					Dialogue.closeNoContinueDialogue(player);
					player.getDialogueManager().startDialogue("SimpleNPCMessage", 943, "Well done! You can cook food on a fire. If you're ever injured, eating food will restore your health. We'll need something to cook. There are shrimp in the pond, so let's catch and cook some. Wherever you see bubbles in the water, there's probably some good fishing to be had there!");
				}
				player.getSkillTasks().decreaseTask(SkillTasks.FNORMAL1);
				player.getSkillTasks().decreaseTask(SkillTasks.FNORMAL2);
			} else if (fire == Fire.OAK) {
	    	...
	    	player.getPackets().sendGameMessage("The fire catches and the logs begin to burn; logs burned "
				+ Colors.RED+Utils.getFormattedNumber(player.getStatistics().getLogsBurned())+"</col>.", true);
...
public boolean start(Player player) {
	...
	World.addGroundItem(new Item(fire.getLogId(), 1), new WorldTile(player), player, true, 60);
