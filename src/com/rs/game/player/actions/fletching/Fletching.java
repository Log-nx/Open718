...
public class Fletching extends Action {
	...
	public static enum Fletch {
		...
		OAK_BOW(1521, KNIFE, new int[] { 54, 56, 9442 }, new int[] { 20, 25, 24 }, new double[] { 16.5, 25, 16 }, new Animation(24940)),

		WILLOW_BOW(1519, KNIFE, new int[] { 60, 58, 9444 }, new int[] { 35, 40, 39 }, new double[] { 33.3, 41.5, 22 }, new Animation(24943)),

		MAPLE_BOW(1517, KNIFE, new int[] { 64, 62, 9448 }, new int[] { 50, 55, 54 }, new double[] { 50, 58.3, 32 }, new Animation(24943)),

		YEW_BOW(1515, KNIFE, new int[] { 68, 66, 9452 }, new int[] { 65, 70, 69 }, new double[] { 67.5, 75, 50 }, new Animation(24943)),

		MAGIC_BOW(1513, KNIFE, new int[] { 72, 70, 25483 }, new int[] { 80, 85, 92 }, new double[] { 83.25, 91.5, 100 }, new Animation(24939)),
		...
		STRUNG_WILLOW_SHORT_BOW(60, 1777, new int[] { 849 }, new int[] { 35 }, new double[] { 33.3 }, new Animation(24947)),

		STRUNG_WILLOW_LONG_BOW(58, 1777, new int[] { 847 }, new int[] { 40 }, new double[] { 41.5 }, new Animation(24954)),

		STRUNG_MAPLE_SHORT_BOW(64, 1777, new int[] { 853 }, new int[] { 50 }, new double[] { 50 }, new Animation(24948)),

		STRUNG_MAPLE_LONG_BOW(62, 1777, new int[] { 851 }, new int[] { 55 }, new double[] { 58.3 }, new Animation(24955)),
		...
		STRUNG_MAGIC_SHORT_BOW(72, 1777, new int[] { 861 }, new int[] { 80 }, new double[] { 83.25 }, new Animation(24950)),

		STRUNG_MAGIC_LONG_BOW(70, 1777, new int[] { 859 }, new int[] { 85 }, new double[] { 91.5 }, new Animation(24957)),
		...
		DRAGON_BANE_BOLTS(24127, 21843, new int[] { 21660 }, new int[] { 85 }, new double[] { 15.6 }, new Animation(19516)),
	...
	public boolean process(Player player) {
		...
		if (fletch.getSelected() == KNIFE && !(player.getInventory().containsItemToolBelt(KNIFE, amount) || player.getInventory().containsItemToolBelt(32635, amount) || player.getEquipment().getWeaponId() == 32635)) {
			player.sm("You need a knife to fletch this.");
			return false;
		} else if (fletch.getSelected() == CHISLE && !player.getInventory().containsItemToolBelt(CHISLE, amount)) {
			player.sm("You need a chisel to fletch this.");
			return false;
		} else if (!player.getInventory().containsItemToolBelt(fletch.getSelected(), amount)) {
			player.sm("You're missing a " + new Item(fletch.getSelected(), amount2).getName().toLowerCase() + ".");
			return false;
		}
		if (!player.getInventory().containsItemToolBelt(fletch.getId(), amount2)) {
			player.sm("You're missing a " + new Item(fletch.getId(), amount2).getName().toLowerCase() + ".");
			return false;
		}
	...
	public int processWithDelay(Player player) {
		...
		int amount3 = ItemDefinitions.getItemDefinitions(fletch == Fletch.SAGIE_SHAFTS ? 5 : fletch.getProduct()[option]).isStackable() ? 15
						: 1;
		if (stackable1) {
			int amt1 = player.getInventory().getAmountOf(fletch.getSelected());
			int amt2 = player.getInventory().getAmountOf(fletch.getId());
		...
		if (fletch == Fletch.STRUNG_SHORT_BOW || fletch == Fletch.STRUNG_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BNORMAL1);
			player.getSkillTasks().decreaseTask(SkillTasks.BNORMAL2);
		} else if (fletch == Fletch.STRUNG_OAK_SHORT_BOW || fletch == Fletch.STRUNG_OAK_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BOAK1);
			player.getSkillTasks().decreaseTask(SkillTasks.BOAK2);
		} else if (fletch == Fletch.STRUNG_WILLOW_SHORT_BOW || fletch == Fletch.STRUNG_WILLOW_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BWILLOW1);
			player.getSkillTasks().decreaseTask(SkillTasks.BWILLOW2);
			player.getSkillTasks().decreaseTask(SkillTasks.BWILLOW3);
		} else if (fletch == Fletch.STRUNG_MAPLE_SHORT_BOW || fletch == Fletch.STRUNG_MAPLE_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BMAPLE1);
			player.getSkillTasks().decreaseTask(SkillTasks.BMAPLE2);
		} else if (fletch == Fletch.STRUNG_YEW_SHORT_BOW || fletch == Fletch.STRUNG_YEW_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BYEW1);
			player.getSkillTasks().decreaseTask(SkillTasks.BYEW2);
			player.getSkillTasks().decreaseTask(SkillTasks.BYEW3);
		} else if (fletch == Fletch.STRUNG_MAGIC_SHORT_BOW || fletch == Fletch.STRUNG_MAGIC_LONG_BOW) {
			player.getSkillTasks().decreaseTask(SkillTasks.BMAGIC1);
			player.getSkillTasks().decreaseTask(SkillTasks.BMAGIC2);
		} else if (fletch == Fletch.RUNITE_BOLT) {
			player.getSkillTasks().decreaseTask(SkillTasks.BRUNE1);
			player.getSkillTasks().decreaseTask(SkillTasks.BRUNE2);
		}
		...
		player.getStatistics().addItemsFletched();
		String name = new Item(fletch.getProduct()[option]).getDefinitions().getName().replace("(u)", "");
		player.getPackets().sendGameMessage("You successfully create "
				...
				+ Utils.getFormattedNumber(player.getStatistics().getItemsFletched()) + "</col>.", true);
