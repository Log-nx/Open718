...
public class Herblore extends Action {
	...
	public boolean process(Player player) {
		...
		if (Utils.random(500) == 0) {
			...
			player.getPackets().sendGameMessage("<col=ff0000>Eli appears from nowhere.", true);
		}
		return true;
	}
	...
	public int processWithDelay(Player player) {
		if (node.getId() == PESTLE_AND_MORTAR || otherItem.getId() == PESTLE_AND_MORTAR) {
			...
			if (player.getPerkManager().hasPerk(PlayerPerks.EXPLOSIVE_WOODCUTTING)) { //change me
		...
		if (otherItem.getId() == SWAMP_TAR || node.getId() == SWAMP_TAR) {
			player.getPackets().sendGameMessage("You add the " + otherItem.getDefinitions().getName().toLowerCase().replace("clean ", "")
					+ " on the swamp tar.", true);
		} else if (otherItem.getId() == PESTLE_AND_MORTAR || node.getId() == PESTLE_AND_MORTAR) {
			player.getPackets().sendGameMessage("You crush the " + otherItem.getDefinitions().getName().toLowerCase()
					+ " with your pestle and mortar.", true);
		} else if (ingredients == Ingredients.TORSTOL && otherItem.getId() != VIAL) {
			player.getStatistics().addPotionsMade();
			player.getPackets().sendGameMessage("You combine the torstol with the potions and get an overload; " + "potions made: "
					+ Colors.RED + Utils.getFormattedNumber(player.getStatistics().getPotionsMade()) + "</col>.", true);
			...
			player.getStatistics().addPotionsMade();
			player.getPackets().sendGameMessage("You mix the " + otherItem.getDefinitions().getName().toLowerCase()
					+ " into your potion; potions made: " + Colors.RED
					+ Utils.getFormattedNumber(player.getStatistics().getPotionsMade()) + "</col>.", true);
		}
	...
	public boolean start(Player player) {
		...
		if (otherItem.getId() == PESTLE_AND_MORTAR || node.getId() == PESTLE_AND_MORTAR) {
			if (!player.getInventory().containsOneItem(PESTLE_AND_MORTAR)) {
				player.getPackets().sendGameMessage("You need a pestle and mortar in order to do this.");
				return false;
			}
		}
		if (ingredients != null) {
