...
public class Prayer implements Serializable {
	...
	public void closePrayers(int prayerId) {
		if (ancientcurses) {
			if (prayerId == 1) {
				...
				if (leechBonuses[6] > 0) {
					player.getPackets().sendGameMessage("Your Defence is now unaffected by sap and leech curses.",
							true);
				}
				adjustStat(2, 0);
				leechBonuses[6] = 0;
				...
				if (leechBonuses[7] > 0) {
					player.getPackets().sendGameMessage("Your Strength is now unaffected by sap and leech curses.",
							true);
				}
				adjustStat(1, 0);
				leechBonuses[7] = 0;
	...
	public void drainPrayer(int amount) {
		if (player.getPerkManager().hasPerk(PlayerPerks.PRAYER_BETRAYER) && !player.isCanPvp()) {
			if (Utils.random(6) < 3) {
				return;
			}
			if (amount > 2) {
	...
	public void drainPrayerOnHalf() {
		...
		if (prayerpoints > 0) {
			prayerpoints = prayerpoints / (player.getPerkManager().hasPerk(PlayerPerks.PRAYER_BETRAYER) ? 4 : 2);
			refreshPrayerPoints();
		}
	}
	...
	public void increaseTurmoilBonus(NPC npc) {
		if (npc.getBonuses() != null) {
			leechBonuses[8] = (int) (100 * Math.floor(0.15 * npc.getBonuses()[0]) / npc.getBonuses()[0]);
			leechBonuses[9] = (int) (100 * Math.floor(0.15 * npc.getBonuses()[9]) / npc.getBonuses()[9]);
			leechBonuses[10] = (int) (100 * Math.floor(0.1 * npc.getBonuses()[6]) / npc.getBonuses()[6]);
	...
	public void increaseTurmoilBonus(Player p2) {
		leechBonuses[8] = (int) (100 * Math.floor(0.15 * p2.getSkills().getLevelForXp(Skills.ATTACK))
				/ p2.getSkills().getLevelForXp(Skills.ATTACK));
		leechBonuses[9] = (int) (100 * Math.floor(0.15 * p2.getSkills().getLevelForXp(Skills.DEFENCE))
				/ p2.getSkills().getLevelForXp(Skills.DEFENCE));
		leechBonuses[10] = (int) (100 * Math.floor(0.1 * p2.getSkills().getLevelForXp(Skills.STRENGTH))
				/ p2.getSkills().getLevelForXp(Skills.STRENGTH));
		adjustStat(0, leechBonuses[8]);
		adjustStat(1, leechBonuses[10]);
		adjustStat(2, leechBonuses[9]);
	}
	...
	private boolean usePrayer(int prayerId) {
		...
		if (getPrayerBook() == 0 && prayerId == 26) {
			if (!player.hasRenewalActivated()) {
				player.getPackets().sendGameMessage("You need to activate the scroll of renewal to use this prayer.");
				return false;
			}
		}
		if (getPrayerBook() == 0 && prayerId == 28) {
			if (!player.hasRigourActivated()) {
				player.getPackets().sendGameMessage("You need to activate the scroll of rigour to use this prayer.");
				return false;
			}
		...
		if (getPrayerBook() == 0 && prayerId == 29) {
			if (!player.hasAuguryActivated()) {
				player.getPackets().sendGameMessage("You need to activate the scroll of augury to use this prayer.");
				return false;
			}
			if (player.getSkills().getLevelForXp(Skills.DEFENCE) < 70) {
				player.getPackets().sendGameMessage("You need a defence level of at least 70 to use this prayer.");
		...
		if (getPrayerBook() == 0) {
			...
			case 20:
				closePrayers(closePrayers[getPrayerBook()][1], closePrayers[getPrayerBook()][2],
						closePrayers[getPrayerBook()][3], closePrayers[getPrayerBook()][10]);
				break;
			case 4:
			case 12:
			case 21:
	...
	public boolean isProtectingItem() {
		if (usingPrayer(0, 10) || usingPrayer(1, 0)) {
			return true;
		}
		return false;
	}
