...
public class SerenStoneCleansing extends Action {
    ...
    public static boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.PRAYER) < 70) {
			player.sm("You need a Prayer level of at least 70 to do this activity.");
		    return false;
		}
		if (!player.getInventory().contains(new Item(32615, 1))) {
    ...
    public int processWithDelay(Player player) {
		ticks --;
		if (!player.getInventory().contains(new Item(32615, 1))) {
			player.sm("You have no more cleansing crystals to work with.");
			this.stop(player);
		}
