...
public class SiphonActionNodes extends Action {
    ...
    private static final WorldTile[][] NODE_TILES = {
	    { new WorldTile(4337, 6076, 1), new WorldTile(4327, 6067, 1),
		    new WorldTile(4338, 6062, 1), new WorldTile(4359, 6117, 1),
		    new WorldTile(4379, 6058, 1), new WorldTile(4385, 6089, 1),
		    new WorldTile(4322, 6091, 1), new WorldTile(4410, 6073, 1) },

	    { new WorldTile(3933, 6122, 1), new WorldTile(3981, 6139, 1),
		    new WorldTile(3958, 6140, 1), new WorldTile(3931, 6139, 1),
		    new WorldTile(3919, 6133, 1), new WorldTile(3912, 6082, 1),
		    new WorldTile(3953, 6066, 1), new WorldTile(3982, 6064, 1),
		    new WorldTile(4018, 6071, 1), new WorldTile(4136, 6136, 1),
		    new WorldTile(4150, 6134, 1), new WorldTile(4188, 6122, 1),
		    new WorldTile(4213, 6047, 1), new WorldTile(4190, 6027, 1),
		    new WorldTile(4161, 6027, 1), new WorldTile(4165, 6027, 1),
		    new WorldTile(4149, 6017, 1), new WorldTile(4133, 6022, 1),
		    new WorldTile(4187, 6122, 1), new WorldTile(4172, 6137, 1),
		    new WorldTile(4139, 6035, 1), new WorldTile(4141, 6082, 1) } };
	...
	player.getActionManager().setAction(
		new SiphonActionNodes(node, object));
	return true;
    }
	...
	for (int j = 0; j < 5; j++) {
		...
		World.spawnObject(new WorldObject(getRandomNodeId(), 10, 0,
			tile.getX(), tile.getY(), tile.getPlane(),
			getFixedLife(node)));
	    }
	}
	Logger.log("Initiated all RuneSpan objects.");
    }

    public static WorldTile getNodeWorldTile(Nodes node) {
	WorldTile tile = NODE_TILES[Utils.random(NODE_TILES.length)][node
    ...
    private void processNodeDestroy(final Player player) {
    	player.sm("The node you were siphoning from has been depleted of energy.");
    	WorldTasksManager.schedule(new WorldTask() {
    		...
    		public void run() {
    			...
    			World.spawnObject(new WorldObject(getRandomNodeId(), node.getType(), node
    					.getRotation(), tile.getX(), tile.getY(), tile.getPlane(), getFixedLife(nodes)));
    			stop();
    		}
    	}, 2);
    }
    		...
    		if (node.getLife() <= 0) {
    			...
    			if (level < 50 ? Utils.getRandom(2) == 1 : Utils.getRandom(1) == 1) {
    				...
    				if (player.getControllerManager().getController() != null 
    						&& player.getControllerManager().getController() instanceof RunespanControler) {
