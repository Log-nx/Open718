...
public class SlayerManager implements Serializable {
	...
	public void getTask() {
		if (player.getSlayerManager().getCurrentTask() == null) {
			...
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 9085, "Your new assignment is "
					+ player.getSlayerManager().getCurrentTask().getName()
					+ ": only " + player.getSlayerManager().getCount()
					+ " more to go.");
		} else {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 9085, "You currently have a slayer task. Come back once you have completed it.");
		}
	}
	...
	private void addPoints() {
		...
		slayerPoints += (pointsIncreased * 2);
		player.sm("Congratulations! You just gained " + (pointsIncreased * 2) + " Slayer points.");
		player.sm("You now have " + slayerPoints + " Slayer points.");
	}

	private boolean removePoints(int pointsValue) {
		int newPoints = slayerPoints - pointsValue;
		if (newPoints < 0) {
			player.sm("You don't have enough points to complete this transaction.");
			return false;
		}
	...
	private void unlockAbility(int slot) {
		...
		player.sm("You have unlocked the ability " + ABILITY[slot][3] + "!");
	}
		...
		if (interfaceId == BUY_INTERFACE) {
			if (componentId == 24 || componentId == 32) {
				if (removePoints(400)) {
					...
					// good enough
					player.sm("You begin to feel wiser and more experienced than before.");
				}
			} else if (componentId == 26 || componentId == 33) {
				if (removePoints(75)) {
					player.getInventory().addItemMoneyPouch(13281, 1);
					player.sm("The master quickly forges you a fully charged ring of slaying.");
				}
			} else if (componentId == 28 || componentId == 36) {
				if (removePoints(35)) {
					player.getInventory().addItemMoneyPouch(560, 250);
					player.getInventory().addItemMoneyPouch(558, 1000);
					player.sm("Here are your runes. Use them wisely.");
				}
			} else if (componentId == 37 || componentId == 34) {
				if (removePoints(35)) {
					player.getInventory().addItemMoneyPouch(13280, 250);
					player.sm("Here are your bolts. Use them wisely.");
				}
			} else if (componentId == 39 || componentId == 35) {
				if (removePoints(35)) {
					player.getInventory().addItemMoneyPouch(4160, 250);
					player.sm("Here are your bolts. Use them wisely.");
				}
			}
		} else if (interfaceId == ABILITIES_INTERFACE) {
	...
	private void cancelCurrentTask() {
		if (currentTask == null) {
			player.sm("You don't have an active task to cancel.");
		} else {
			if (removePoints(30)) {
				skipCurrentTask();
				setCurrentTask(true);
				player.sm("Your slayer task has been re-assigned as requested, as a result, your slayer-streak has been reset to 0.");
			}
		}
	}
	...
	private void addRemovedTask(int slot) {
		...
		sendSlayerInterface(ASSIGNMENT_INTERFACE);
		player.sm("You have re-added " + task.getName().toLowerCase() + " to the assignment list.");
	}

	private void removeCurrentTask() {
		if (canceledTasksCount != canceledTasks.length) {
			if (currentTask == null) {
				player.sm("You don't have an active task to remove.");
			} else {
				if (removePoints(100)) {
					for (int index = 0; index < 6; index++) {
						...
						if (task == null) {
							...
							player.sm("You have cancled the task " + currentTask.getName() + " permenantly.");
							skipCurrentTask();
							sendSlayerInterface(ASSIGNMENT_INTERFACE);
							return;
						}
			...
			player.sm("You have reached the maximum limit of cancelable tasks, please remove one before continueing.");
	}
	...
	public boolean isValidTask(NPC npc, String name) {
		if (currentTask == null) {
			return false;
		}
		...
		String currentTaskName = currentTask.getName().toLowerCase();

		
		if (npc.getId() == 9172 && currentTaskName.equalsIgnoreCase("aquanites")) {
			return true;
		}
		
		if (npc.getId() == 6257 && currentTaskName.equalsIgnoreCase("spiritual mage")) {
			return true;
		}
	...
	public void checkCompletedTask(int damageAdmitted, int otherDamageAdmitted) {
		...
		if (currentTaskCount + otherSocialCount == maximumTaskCount) {
			player.kuradalTasks++;
			if (socialPlayer != null)
				socialPlayer.sm("You have finished your slayer task, talk to a slayer master for a new one.");
			player.sm("You have finished your slayer task, talk to a slayer master for a new one.");
			resetTask(true, true);
		} else if (currentTaskCount % 10 == 0)
			checkKillsLeft();
	}

	public static boolean handleGem(Player player, int itemId, int option) {
		if (itemId == 4155) {
			if (option == 1)
	...
	public void checkKillsLeft() {
		if (currentTask == null) {
			player.sm("You currently have no slayer task assigned.");
			return;
		}
		player.sm("Your current assignment is: " + currentTask.getName() + "; only " + getCount() + " more to go.");
		if (socialPlayer != null) {
			player.sm("Your partner's current assignment is: " + currentTask.getName() + "; only " + player.getSlayerManager().getCount() + " more to go.");
			int combinedTasksCount = currentTaskCount + socialPlayer.getSlayerManager().getCurrentTaskCount();
			player.sm("In total you both have killed " + combinedTasksCount + " out of " + maximumTaskCount + " of the task, only " + (maximumTaskCount - combinedTasksCount));
		}
	}
	...
	public void invitePlayer(Player otherPlayer) {
		if (currentTask != null) {
			player.sm("You need to complete your current task before starting a social slayer group.");
			return;
		} else if (otherPlayer == null || !otherPlayer.withinDistance(player, 7) || player.hasFinished() || otherPlayer.hasFinished()) {
			player.sm("Your target is no-where to be found.");
			return;
		} else if (otherPlayer.getSlayerManager().getCurrentTask() != null) {
			player.sm("Your target needs to complete their current task before joining a social slayer group.");
			return;
		} else if (otherPlayer.getSlayerManager().getSocialPlayer() != null) {
			player.sm("Your target is already in a social slayer group.");
			return;
		} else if (socialPlayer != null) {
			player.sm("You are already in a social slayer group, leave it in order to start a new one.");
			return;
		}
		...
		player.getTemporaryAttributtes().put("social_request", otherPlayer);
		player.sm("Sending " + otherPlayer.getDisplayName() + " an invitation...");
		otherPlayer.getPackets().sendMessage(117, "You have received an invitation from the player " + player.getDisplayName() + " for either social slayer or clan recruitement.", player);
	}
	...
	private void openSocialInvitation(final Player otherPlayer) {
		...
		player.setCloseInterfacesEvent(new Runnable() {
			...
			@Override
			public void run() {
				otherPlayer.sm("Your invitation has been declined.");
				player.sm("You have declined the invitation.");
			}
		});
	}
	...
	public void createSocialGroup(boolean initial) {
		...
		if (initial) {
			...
			player.sm("You have created a social group.");
		} else
			player.sm("You have just joined " + socialPlayer.getDisplayName() + "'s social group.");
	...
	public void setCurrentTask(boolean initial) {
		player.getSlayerManager().setCurrentMaster(Slayer.SlayerMaster.getSlayerMasterForLevels(player));
