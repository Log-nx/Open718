...
public class SlayerTask implements Serializable {
	...
	public enum Master {
		KURADAL(9085, new Object[][] { { "Rock crab", 1, 20, 10, 20, 10.72 }, { "Crawling hand", 5, 35, 15, 30, 35.81 },
				...
				{ "Airut", 92, 100, 100, 160, 260.05 }, { "Kal'gerion Demon", 90, 100, 100, 160, 235 },
	...
	public static SlayerTask random(Player player, Master master, boolean resetTask) {
		...
		while (true) {
			...
			int maxLevel = (Integer) master.data[random][2];
			if (player.getSkills().getLevel(Skills.SLAYER) < requiredLevel
					|| player.getSkills().getLevel(Skills.SLAYER) > maxLevel) {
				continue;
			}
			int minimum = (Integer) master.data[random][3];
			int maximum = (Integer) master.data[random][4];
	...
	private static int givePoints(Player player) {
		...
		if (player.getPerkManager().hasPerk(PlayerPerks.SLAYER_BATTLE_CRY)) {
			...
			if (npc.getDefinitions().name.toLowerCase().contains(killer.getTask().getName(killer).toLowerCase())) {
				...
				if (killer.getTask().getTaskAmount() <= 0) {
					killer.getSkills().addXp(Skills.SLAYER,
							lvl * 9.1 * (killer.getPerkManager().hasPerk(PlayerPerks.SLAYER_BATTLE_CRY) ? 1.25 : 1));
				...
				if (npc.getDefinitions().name.toLowerCase()
					...
					if (slayerPartner.getTask().getTaskAmount() <= 0) {
						...
						slayerPartner.getSkills().addXp(Skills.SLAYER, lvl * 9.1 / (withinDistance ? 2 : 5)
								* (killer.getPerkManager().hasPerk(PlayerPerks.SLAYER_BATTLE_CRY) ? 1.25 : 1));
	 ...
	 */
	public static boolean handleShop(Player player, int interfaceId, int componentId) {
		if (interfaceId == 164) {
			if (componentId == 16) {
				sendLearn(player);
			}
		...
		if (interfaceId == 378) {
			...
			if (componentId == 75) {
				player.getDialogueManager().startDialogue("SlayerRewards", "gpcollector", 500);
				return false;
			}
			if (componentId == 76) {
			...
			if (componentId == 78) {
				player.getDialogueManager().startDialogue("SlayerRewards", "charmingimp", 500);
				return false;
			}
			player.getInterfaceManager().closeChatBoxInterface();
		}
	...
	public static void sendLearn(Player player) {
		...
		player.getPackets().sendItemOnIComponent(378, 102, 15585, 1); // LAST
																		// SLOT
		...
		player.getPackets().sendItemOnIComponent(378, 96, 6735, 1); // FIFTH
																	// SLOT END
		...
		player.getPackets().sendItemOnIComponent(378, 105, 32625, 1); // SECOND
																		// SLOT
		player.getPackets().sendItemOnIComponent(378, 101, 27996, 1); // THIRD
																		// SLOT
		...
		player.getPackets().sendIComponentText(378, 86, "Coin Accumulator");
		player.getPackets().sendIComponentText(378, 97, "300 points");
		player.getPackets().sendIComponentText(378, 98, "500 points");
		...
		player.getPackets().sendIComponentText(378, 88, "Charming Imp");
		player.getPackets().sendIComponentText(378, 100, "500 points");
