...
public class SpiritShieldCreation {
	...
	public static void attachShield(Player player, Item sigil, boolean smith) {
		if (!player.getInventory().containsItem(BLESSED_SPIRIT_SHIELD, 1)) {
			player.getPackets().sendGameMessage("You need a spirit shield to attach sigil to it.");
			return;
		}
		if (!smith) {
			if (!player.getInventory().containsItemToolBelt(Smithing.HAMMER)) {
				player.getDialogueManager().startDialogue("SimpleMessage", "You need a hammer in order to work with spirit shields.");
				return;
			}
			if (player.getSkills().getLevel(Skills.SMITHING) < 85) {
				player.getDialogueManager().startDialogue("SimpleMessage", "You need a Smithing level of 85 to attach sigil to the blessed spirit shield.");
				return;
			}
			if (player.getSkills().getLevel(Skills.PRAYER) < 90) {
				player.getPackets().sendGameMessage("You need 90 prayer to to attach sigil to the blessed spirit shield.");
				return;
			}
		...
		player.getDialogueManager().startDialogue("SimpleMessage", (smith ? "You" : "Brother Bordiss") + " succesfuly attach the sigil to the blessed spirit shield.");
	...
	public static void blessShield(Player player, boolean altar) {
		if (!player.getInventory().containsItem(HOLY_ELIXIR, 1) || !player.getInventory().containsItem(SPIRIT_SHIELD, 1)) {
			player.getPackets().sendGameMessage("You need a spirit shield and a holy elixir to bless it.");
			return;
		}
		if (!altar) {
			if (player.getInventory().getCoinsAmount() < 1000000) {
				player.getPackets().sendGameMessage("Brother Jered wants 1m to bless the spirit shield.");
				return;
			}
			player.getInventory().removeItemMoneyPouch(new Item(995, 1000000));
		} else {
			if (player.getSkills().getLevel(Skills.PRAYER) < 85) {
				player.getPackets().sendGameMessage("You need 85 prayer to bless the spirit shield.");
				return;
			}
		}
		player.lock(2);
		...
		player.getDialogueManager().startDialogue("SimpleMessage", (altar ? "You" : "Brother Jered") + " succesfully bless the shield using the holy elixir and the powers of Saradomin.");
