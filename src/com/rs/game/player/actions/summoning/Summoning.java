...
import com.rs.game.npc.others.MemstixNPC;
...
public class Summoning implements Serializable {
	...
	private static final long serialVersionUID = 5444414962055070170L;

	public static final class Pouches implements Serializable {

		private static final long serialVersionUID = 4474598479713902609L;
		...
		public static final Pouches ASSASSIN;
		...
		static {
		    SPIRIT_WOLF = new Pouches("SPIRIT_WOLF", 0, 6830, 67, 12047, 1, 0.10000000000000001D, 4.7999999999999998D, 0x57e40L, 1,  12425);
		    DREADFOWL = new Pouches("DREADFOWL", 1, 6825, 69, 12043, 4, 0.10000000000000001D, 9.3000000000000007D, 0x3a980L, 1,  12445);
		    SPIRIT_SPIDER = new Pouches("SPIRIT_SPIDER", 2, 6842, 83, 12059, 8,
		    ...
		    FRUIT_BAT = new Pouches("FRUIT_BAT", 52, 6817, 79, 12033, 69, 1.3999999999999999D, 121.2D, 0x2932e0L, 7, 12423);
		    

		    RAVENOUS_LOCUST = new Pouches("RAVENOUS_LOCUST", 53, 7374, 97,
			    12820, 70, 1.5D, 132D, 0x15f900L, 4, 12830);
		    ARCTIC_BEAR = new Pouches("ARCTIC_BEAR", 54, 6839, 109, 12057, 71,
			    1.1000000000000001D, 93.200000000000003D, 0x19a280L, 8, 12451);
		   ...
		   ASSASSIN = new Pouches("ASSASSIN", 77, 8423, 163, 15073, 1, 4.9000000000000004D, 435.19999999999999D, 0x3a9800L, 10, 12825);
		    ...
		    ENUM$VALUES = (new Pouches[] { SPIRIT_WOLF, DREADFOWL,
			    ...
			    BLUEDRAGON, GREENDRAGON, BLACKDRAGON, WILYHELLCAT, HELLCAT, ASSASSIN });
		...
		private Pouches(String s, int i, int npcId, int configId, int pouchId, int level, 
				double useExp, double creationExp, long time, int spawnCost, int scrollId) {
		    super();
		    this.npcId = npcId;
    ...
    public static void sendInterface(Player player) {
		...
		final Object[] options = new Object[] { 78, 1, "List<col=FF9040>", "Infuse-X<col=FF9040>", 
				"Infuse-All<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-5<col=FF9040>", 
    ...
    public static Familiar createFamiliar(Player player, Pouches pouch) {
		...
		try {
		    Familiar fam = (Familiar) Class.forName(loc+ NPCDefinitions.getNPCDefinitions(pouch
						    .getNpcId()).name.replace(" ", "").replace("-", "")).getConstructor(
				    new Class[] { Player.class, Pouches.class,
					    WorldTile.class, Integer.TYPE, Boolean.TYPE }).newInstance(
				    new Object[] { player, pouch, player, Integer.valueOf(-1), Boolean.valueOf(true) });
		    if (fam != null) {
		    	return fam;
		    }
		} catch (Throwable e) {
		    //e.printStackTrace();
			System.err.println("Unsupported familiar: "+pouch+"");
		}
		return null;
    }
    
    private static int getRequiredLevel(int id) {
    ...
    public static void spawnFamiliar(Player player, Pouches pouch) {
		...
		if (!player.getControllerManager().canSummonFamiliar())
			return;
		final ItemDefinitions def = ItemDefinitions.getItemDefinitions(pouch.getPouchId());
		if (def == null)
			return;
		int levelReq = getRequiredLevel(pouch.getPouchId());
		...
		if (player.getSkills().getLevel(Skills.SUMMONING) < pouch.getSpawnCost()) {
			player.sendMessage("You do not have enough Summoning points to spawn this familiar.");
			return;
		}
		if (player.getCurrentFriendChat() != null) {
			final ClanWars war = player.getCurrentFriendChat().getClanWars();
			if (war != null) {
				if (war.get(Rules.NO_FAMILIARS) && (war.getFirstPlayers().contains(player) 
						|| war.getSecondPlayers().contains(player))) {
					player.sendMessage("You can't summon familiars during this war.");
					return;
				}
			}
		}
    ...
    public static int amountCanCreate(final Player player, int itemId,  int amount) {
    ...
    public static void listRequirements(final Player player, int itemId, int amount) {
    ...
    public static void createPouch(final Player player, int itemId, int amount) {
		try {
			...
			final SummoningPouch pouch = SummoningPouch.get(itemId);
			if (pouch == null) {
				player.sendMessage("This pouch is currently unavailable. Please try back at a later time.");
				return;
			}
			if (amount <= 0)
				return;
			...
			if (player.getSkills().getLevelForXp(Skills.SUMMONING) < pouch.getLevelRequired()) {
				player.sendMessage("You don't have the level required to create that pouch.");
				return;
			}
			if (!player.getInventory().containsItems(pouch.getItems())) {
				player.sendMessage("You don't have enough materials to do this.");
				return;
			}
			if (amount > player.getInventory().getAmountOf(pouch.getItems()[2].getId()))
				amount = player.getInventory().getAmountOf(pouch.getItems()[2].getId());
			for (final Item i : pouch.getItems()) {
				final int amountCanMake = (int) Math.floor(player.getInventory().getNumberOf(i.getId()) / i.getAmount());
				if (amountCanMake < amount)
					amount = amountCanMake;
			}
			player.getInterfaceManager().closeScreenInterface();
			for (final Item i : pouch.getItems()) {
				//Just for stackable ingredient, temp fix
				if (i.getId() == 6319 || i.getId() == 10033) {
					if (player.getInventory().getFreeSlots() < amount) {
						amount = player.getInventory().getFreeSlots();
						if (amount < 1)
							player.sendMessage("Inventory full. To make some room, sell, drop or bank something.");
					}
				}
			}
			...
			for (final Item i : pouch.getItems())
				player.getInventory().deleteItem(i.getId(), i.getAmount() * amount);

			player.getInventory().addItemDrop(pouch.getPouchId(), amount);
			final double experience = pouch.getCreateExperience() * amount;
			player.getSkills().addXp(Skills.SUMMONING, experience / 1.5);
			player.getPackets().sendObjectAnimation(new WorldObject(28716, 10, 1, 2209, 5344, 0), new Animation(8509));
			player.getStatistics().setPouchesMade(player.getStatistics().getPouchesMade() + amount);
			player.sendMessage("You infuse " + (amount == 1 ? "a" : "some") + " "
					+ item.getName().toLowerCase() + (amount == 1 ? "" : "es") + "; pouches made: "
							+ Colors.RED+Utils.getFormattedNumber(player.getStatistics().getPouchesMade())+"</col>.");
			player.setNextAnimation(new Animation(9068));
			if (Utils.random(75 - amount) == 0) {
		    	new MemstixNPC(player, player);
				player.sendMessage("<col=ff0000>A Mysterious Summoner appears from the obelisk.");
		    }
    ...
    public static double shamanSuit(Player player) {
    	...
    	if (player.getEquipment().getGlovesId() == 28998)
    		xpBoost *= 1.01;
    	if (player.getEquipment().getHatId() == 28995
    			&& player.getEquipment().getChestId() == 28996
    			&& player.getEquipment().getLegsId() == 28997
    			&& player.getEquipment().getBootsId() == 28999
    			&& player.getEquipment().getGlovesId() == 28998)
    		xpBoost *= 1.01;
    	return xpBoost;
    }
}