...
import com.rs.game.npc.random.RogueNPC;
...
public class PickPocketAction extends Action {
    ...
    private boolean checkAll(Player player) {
		...
		if (!player.getInventory().hasFreeSlots()) {
		    player.getPackets().sendGameMessage("You don't have enough space in your inventory.", true);
		    return false;
		}
		if (player.getAttackedBy() != null && player.getAttackedByDelay() > Utils.currentTimeMillis()) {
		    player.getPackets().sendGameMessage("You can't do this while you're under combat.", true);
		    return false;
		}
    ...
    private int getIncreasedChance(Player player) {
		...
		if (player.getPerkManager().hasPerk(PlayerPerks.SLEIGHT_OF_HAND))
			chance += 30;
		return chance;
    }
		...
		case 0:
		    return "You succesfully pick the "
			    + npc.getDefinitions().name.toLowerCase() + "'s pocket; "
	    			+ "times thieved: "+Colors.RED+Utils.getFormattedNumber(player.getStatistics().getTimesStolen())+"</col>.";
		case 1:
    ...
    public int processWithDelay(Player player) {
    	if (!isSuccesfull(player) && !player.getPerkManager().hasPerk(PlayerPerks.SLEIGHT_OF_HAND)) {
			    player.getPackets().sendGameMessage("You fail to pick the "+npc.getDefinitions().name.toLowerCase()+ "'s pocket.", true);
			    player.getPackets().sendGameMessage("Purchase the Sleight of Hand perk to never fail pickpocketing.", true);
			    if (npc.getId() != 1911) {
			    ...
			    for (int i = 0; i <= index; i++) {
			    	...
			    	if (item.getId() == 995)
						...
						player.getGamePointManager().addGamePointItem(item.getId(), item.getAmount());
			    }
			}
		if (Utils.random(75) == 0) {
			new RogueNPC(new WorldTile(player), player);
	    	player.getPackets().sendGameMessage("<col=ff0000>A Rogue appears out of nowhere.");
		}
		return -1;
    }
    ...
    public boolean start(Player player) {
		if (checkAll(player)) {
		    ...
		    player.getPackets().sendGameMessage("You attempt to steal from the " + npc.getDefinitions().name.toLowerCase()+"..", true);
