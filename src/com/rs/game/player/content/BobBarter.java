...
public class BobBarter {
	...
	for (int i = 0; i < potsList.size(); i++) {
	    if (potsList
		    ...
		    .toString()
		    .substring(0,
			    potsList.get(i).get(0).toString().length() - 4)
	...
	if (pots > maxPots) {
	    player.getPackets().sendGameMessage(
		    "You can only decant 1M potions at a time");
	    return;
	}
    }
	...
	for (int i = 0; i < player.getInventory().getItemsContainerSize(); i++) {
		...
		for (int f = 110; f < Utils.getItemDefinitionsSize(); f++) {
		    ...
		    if (item.getName().contains("(" + a + ")")
			    && item.getName().equals(
				    ItemDefinitions.getItemDefinitions(f)
					    .getName())) {
			if (!item.getDefinitions().isNoted()) {
				...
				for (int q = 0; q < item.getAmount(); q++) {
				    potsList.get(potsList.size() - 1).add(
					    itemName);
				}
			    } else {
				for (int q = 0; q < item.getAmount(); q++) {
				    potsList.get(potPlace(itemName)).add(
					    itemName);
				}
			    }
			    totalVials += item.getAmount();
	...
	for (int i = 0; i < potsList.size(); i++) {
	    int vials = 0, totalDoses = 0, partDoses, fullVialId = 0, partVialId = 0, fullVials, partVials = 0, emptyVialId = 0;
		...
		vials++;
		totalDoses += Integer.parseInt(itemName.substring(
			itemName.length() - 2, itemName.length() - 1));
	    }
		...
		if (itemName.contains("(" + x + ")")) {
			...
			partVial = itemName.replace(Integer.toString(x),
				Integer.toString(partDoses));
		    } else {
			emptyVialId = 230;
			fullVial = itemName.substring(1, itemName.length())
				.replace(Integer.toString(x), "4");
			partVial = itemName.substring(1, itemName.length())
				.replace(Integer.toString(x),
					Integer.toString(partDoses));
		    }
		}
	    }
		...
		player.getInventory().addItem(
			new Item(emptyVialId, vials - (fullVials + partVials)));
	    }
	}
    }
