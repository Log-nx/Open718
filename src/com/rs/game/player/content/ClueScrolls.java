...
public class ClueScrolls {
		...
		if (hasRiddleClue(p, scroll.id) != null){
			...
			if (lastloc.getX() >= riddleclue.locations[0] && lastloc.getY() <= riddleclue.locations[1] && lastloc.getX() <= riddleclue.locations[2] && lastloc.getY() >= riddleclue.locations[3]){
				if (emoteid == riddleclue.emoteid){
					...
					p.getInventory().addItemDrop(2717, 1);
				}
			}
		}
		}
	...
	public static boolean digSpot(Player p){
		...
		if (hasMapClue(p, scroll.id) != null){
			...
			if (lastloc.getX() == mapclue.xcoord && lastloc.getY() == mapclue.ycoord){
					p.sm("You have successfully completed the riddle and have been awarded a chest!");
					p.getInventory().deleteItem(scroll.id, 1);
					p.getInventory().addItemDrop(2717, 1);
				}
			}
	...
	*/
	static int[] EasyRewards = {995, 2583, 2585, 2587, 2589,
		...
		10468, 13095, 10398, 10748, 10750, 10752, 10756, 10402,
		10406, 10410, 10414, 10418, 10420, 10424, 10428, 10432,
		10436, 10422, 10426, 10430, 10434, 10438, 10392, 10396, 10394, 10354, 10364, 10366,
		626, 628, 630, 632, 634, 1837, 1833, 1835};

	static int[] MediumRewards = {995, 2599, 14936, 14938, 2601, 2603, 2605,
		2607, 2609, 2611, 2613, 2579, 7319,
		7321, 7323, 7325, 7327, 10364, 10452, 10454,
		10456, 19380, 19382, 19384, 19386, 19388, 193890,
		13107, 13109, 13111, 13113, 13115, 13097, 7668, 12210,
		12213, 12216, 12219, 12222, 13660, 2643, 2639, 2641, 11280, 9005, 9006, 10354,
		2894, 2904, 2914, 2924, 2934};
	...
	static int[] HardRewards = {995, 2615, 2617, 2619, 2621,
		...
		7400, 10330, 10332, 10334, 10336, 10338, 10340,
		10342, 10344, 10346, 10348, 10350, 10352, 10362,
		10470, 10472, 10474, 19392, 19394, 19396, 10440,
		...
		10382, 19281, 19284, 19387, 13099, 9470,
		9471, 22215, 22216, 22217, 22218, 19346, 19348, 
		19350, 19352, 19354, 19356,
		19358, 19360, 25312, 25314, 19333};

	static int[] EliteRewards = {995, 19308, 19311, 19314, 19317,
		19320, 19413, 19416, 19419, 19422, 19425, 19428,
		...
		2577, 2581, 3057, 3058, 3059, 3060, 3061};
	...
	public static void giveReward(Player p){
		...
		if (random > 875){
			int itemCount = Utils.getRandom(3)+3;
			if (p.getInventory().getFreeSlots() < itemCount) {
				p.sm("You don't have enough inventory space to do that.");
				return;
			}
			...
			for (Item item: rewards){
					...
					p.sm("Congratulations, you have won a "+item.getName()+"!");
			...
			p.getInventory().deleteItem(2717,1);
			p.getInventory().addItemDrop(ScrollIds[Utils.getRandom(ScrollIds.length-1)], 1);
			if (p.cluenoreward == 0){
				p.cluenoreward += 1;
			}
