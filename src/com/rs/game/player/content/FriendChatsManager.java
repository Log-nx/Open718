...
public class FriendChatsManager {
    ...
    private void joinChatNoCheck(Player player) {
		synchronized (this) {
		    ...
		    player.getPackets().sendGameMessage("You are now talking in the friends chat channel "  + settings.getChatName(), true);
		    refreshChannel();
		}
    }
	...
	if (chat == null) {
		player.getPackets().sendGameMessage("You need to be in a friends chat channel to use this option.");
		return;
	}
	synchronized (this) {
	...
	synchronized (this) {
	    ...
	    if (!logout) {
		player.setCurrentFriendChatOwner(null);
		 player.getPackets().sendGameMessage("You have left the channel.", true);
		player.toogleLootShare = false;
		player.getPackets().sendFriendsChatChannel();
	    }
	    if (clanWars != null) {
    ...
    public static List<Player> getLootSharingPeople(Player player) {
		...
		for (Player p2 : players) {
			if (p2.isToggleLootshare() && p2.withinDistance(player, 32))
				players.add(p2);
		}
		return players;
	...
	public void toogleLootShare(Player player) {
		...
		if (!player.getUsername().equals(player.getCurrentFriendChat().getOwnerName())
				&& !player.getCurrentFriendChat().settings.hasRankToJoin(player.getUsername())) {
			player.getPackets().sendGameMessage("You must be on channel owner's Friend List to use LootShare on this channel.");
			player.refreshToggleLootShare();
			return;
		}
