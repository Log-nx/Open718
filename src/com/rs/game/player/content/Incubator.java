...
public class Incubator {
	...
	public static boolean useEgg(Player player, int itemId) {
		...
		if (egg == null)
			return false;
		if (player.getSkills().getLevelForXp(Skills.SUMMONING) < egg.summoningLevel) {
			player.getPackets().sendGameMessage("You need a level of "+egg.summoningLevel+" summoning to hatch this egg.");
			return true;
		}
		player.lock(1);
		...
		player.getPackets().sendGameMessage("You put the egg in the incubator and it hatches.");
		return true;
	}
}