...
public class MoneyPouch implements Serializable {
	...
	public void withdrawPouch() {
		if (player.getControllerManager().getController() instanceof DungeonController) {
			player.getPackets()
					.sendGameMessage(
							"You cannot acess your money pouch within the walls of Daemonheim.");
			return;
		}
	...
	public void refreshCoins() {
		...
		player.getPackets().sendItems(623,
				new Item[] { new Item(995, coinAmount, true) });
	}
	...
	public boolean sendDynamicInteraction(int amount, boolean remove, int type) {
		...
		final String FILE_PATH = "data/logs/moneypouch/";
		BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH + player.getUsername() + ".txt", true));
		writer.write("[" + dateFormat.format(cal.getTime()) + " " + player.getUsername() + " | IP: "
				+ player.getSession().getIP() + " ] Amount: " + Utils.getFormattedNumber(amount) + " | Type: " + (remove ? "Remove" : "Add"));
		writer.newLine();
		writer.flush();
		writer.close();
