...
public class PenguinEvent {
	...
	public enum Penguins {
		...
		OOGLOGPORTALS(new WorldTile(2594, 2855, 0), "Maybe around here somewhere.", BARREL),
		RIMMINGTONMINING(new WorldTile(2975, 3248, 0), "This penguin can be found somewhere Oogle City!", ROCK),
		...
		OOGLOGMININGROCK(new WorldTile(2585, 2878, 0), "Can be found around here.", BARREL),
	...
	public static void spotPenguin(Player player, NPC npc) {
		if (!player.penguin) {
			...
			World.sendWorldMessage("[<col=<col=89bdd3>Penguin Event</col>] <col=89bdd3>Congratulations, "+ player.getDisplayName() +" has successfully spot the penguin.", false, false);
			//player.sm("You successfully spot the penguin and have been rewarded 'one' penguin point.");
			//player.sm("You now have '"+player.penguinpoints+"' penguin points, keep it up.");
		} else {
	...
	public static void startEvent() {
		...
		//World.sendWorldMessage("<col=89bdd3>Ooh, there have been transported a penguin in this land, be the first to find the penguin, the person who finds the penguin will be rewarded! Get started now, here's a hint for you: who was transported a penguin in the country, being the first to find the penguin, the person who finds the penguin will be rewarded! get started now, the hint is: "+penguin.getHint()+"", false);
		World.sendWorldMessage("[<col=89bdd3>Penguin Event</col>] <col=89bdd3>Ooh, there have been spawned a penguin, be the first to find it!", false, false);
		World.sendWorldMessage("<col=89bdd3>What's the hint?</col> "+penguin.getHint()+"", false, false);
