...
/**
 * Handles the creation of Summoning Scrolls.
...
public class SummoningScroll {
    ...
    /**
     * Creates the Summoning Scroll.
     * @param player The player creating.
     * @param itemId The ItemID using.
     * @param amount The amount creating.
     */
    public static void createScroll(Player player, int itemId, int amount) {
		...
		if (!player.getInventory().containsItem(scroll.pouchId, 1)) {
		    player.sendMessage("You do not have enough " +  ItemDefinitions.getItemDefinitions(scroll.pouchId).getName().toLowerCase()+ "es for this action.");
		    return;
		}
		if (player.getSkills().getLevelForXp(Skills.SUMMONING) < scroll.reqLevel) {
		    player.sendMessage("You need a summoning level of " + scroll.reqLevel + " to create "   + amount + " "  + ItemDefinitions.getItemDefinitions(scroll.scrollId).getName().toLowerCase() + "s.");
		    return;
		}
		player.getInventory().deleteItem(scroll.pouchId, amount);
		player.getInventory().addItem(scroll.scrollId, amount * 10);
		player.getSkills().addXp(Skills.SUMMONING, scroll.experience * amount);
		player.sendMessage("You have created some "+ ItemDefinitions.getItemDefinitions(scroll.scrollId).getName().toLowerCase() + "s.");
		player.setNextAnimation(new Animation(9068));
		player.closeInterfaces();
    }

    /**
     * Sends the Summoning obelisk interface.
