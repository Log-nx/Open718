package com.rs.game.player.content.activities.reaper;
...
public class Reaper implements Serializable {
	...
	private int setPoints() {
		...
		player.getVarBitManager().sendVarBit(22905, points);
		return this.points = 7;
	}
	...
	public void checkKillsLeft() {
		if (task == null) {
			player.getPackets().sendGameMessage("You have no reaper assignment.");
			return;
		}
		player.getPackets().sendGameMessage("You are currently assigned to collect souls from: " + task.getName() + ".");
		player.getPackets().sendGameMessage("You must retrieve " + getAmount() + " to complete your assignment.");
	}

	public void updateTask() {
		if (getAmount() < (player.getDonationManager().isDonator() ? 4 : 2)) {
			player.getPackets().sendGameMessage("<col=009a00>You have completed Death's assignment, gaining " + Utils.formatNumber(getXp() * player.getXpRate() / 2)
			+ " Slayer XP and " + getPoints() + " reaper points.");
			player.getSkills().addSkillXpRefresh(18, getXp() * player.getXpRate() / 2);
