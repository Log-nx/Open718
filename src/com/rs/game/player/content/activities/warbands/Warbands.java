...
public final class Warbands {
	...
	public enum WarbandEvent {
		CAMP_1(new int[] { 3026, 3040, 3586, 3599 }, 0, new WorldTile(3037, 3592, 0)),

		CAMP_2(new int[] { 3296, 3313, 3762, 3777 }, 1, new WorldTile(3308, 3772, 0)),

		CAMP_3(new int[] { 3128, 3141, 3836, 3852 }, 2, new WorldTile(3132, 3839, 0));
	 ...
	 */
	boolean decreaseRemainingResources(Player player, WorldObject object) {
		...
		if (warband.resources >= 1) {
			...
			if (player.hasStarted() && Utils.getRandom(500) == 1)
				Warbands.giveWandOfTreachery(player);
			player.getPackets().sendGameMessage("You loot the camp.", true);
		}
		return true;
	}

	private static void giveWandOfTreachery(Player player) {
		if (player.getInventory().hasFreeSlots()) {
		player.getInventory().addItem(ItemIdentifiers.WAND_OF_TREACHERY, 1);
		} else {
			player.getBank().addItem(ItemIdentifiers.WAND_OF_TREACHERY, 1, true);
		}
		World.sendWorldMessage("<img=10><col=1c73ec> " + Utils.formatPlayerNameForDisplay(player.getUsername()) + "</col> has found a <col=1c73ec>" + ItemDefinitions.getItemDefinitions(ItemIdentifiers.WAND_OF_TREACHERY).getName().toLowerCase() + "</col> as while participating in Warbands!", false, false);
		new MessageBuilder().append(player.getDisplayName() + " has found a " + ItemDefinitions.getItemDefinitions(ItemIdentifiers.WAND_OF_TREACHERY).getName().toLowerCase() + " while participating in Warbands!").send(GameServer.getDiscordBot().getAPI().getTextChannelById("504474120956936211").get());
	}
	...
	public Warbands(int event) {
		...
		World.sendWarbandsNews("A new " + getWarbandsEventType() + " warband has been spotted somewhere between levels " + getWildLevel(base[2]) + " and " + getWildLevel(base[3]) + " of the wilderness!", 2);
		//if (Settings.HOSTED)
		new MessageBuilder().append("[Warbands] A new " + getWarbandsEventType() + " warband has been spotted somewhere between levels " + getWildLevel(base[2]) + " and " + getWildLevel(base[3]) + " of the wilderness!").send(GameServer.getDiscordBot().getAPI().getTextChannelById("534549522894815236").get());
		warband = this;
		spawnCombatants();
