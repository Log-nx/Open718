...
public class ClansManager {
    ...
    private Clan clan;
    private ArrayList<Player> channelPlayers; 
    private ArrayList<Player> membersOnline;
	...
	if (player.getClanManager() == null)
	    return;
	player.getDialogueManager().startDialogue("ClanLeaving");
    }
	...
	player.getTemporaryAttributtes().put("MotifCustomization", part);
	...
	if (manager == null)
	    return;
	Integer part = (Integer) player.getTemporaryAttributtes().remove("MotifCustomization");
	if (part == null)
	    return;
	...
	public void generateClanChannelDataBlock() {
		...
		for (final Player player : channelPlayers) {
			// stream.writeLong(
	...
	if (manager == null)
	    return;
	synchronized (manager) {
	    if (player.getGuestClanHandler() != null)
		player.getGuestClanHandler().disconnect(player, true);
	...
	clanName = Utils.formatPlayerNameForDisplay(clanName);
	ClansManager manager = guest ? player.getGuestClanHandler() : player.getClanManager();
    ...
    public void sm(Player player, ChatMessage message) {
    	synchronized (this) {
    	    String displayName = player.getDisplayName();
    	    int rights = player.getRights();
    	    for (Player p2 : channelPlayers)
	...
	public static void joinClanChatChannel(Player player) {
		if (player.getClanManager() == null) {
			player.sm("You must be a member of a clan in order to join their channel.");
	...
	if (player.isAnIronMan() && !p2.isAnIronMan()) {
		player.sendMessage("You are only allowed to invite other Ironmen to your clan.");
		return;
	}
	if (!player.isAnIronMan() && p2.isAnIronMan()) {
		player.sendMessage("You are only allowed to invite other Ironmen to your clan.");
	...
	p2.getPackets().sendClanInvitation(player);
	p2.getTemporaryAttributtes().put("claninvite", Boolean.TRUE);
    }
    
    public static void handleButtons(Player player, int interfaceId, int componentId, int slotId) {
    	if (componentId == 41)
