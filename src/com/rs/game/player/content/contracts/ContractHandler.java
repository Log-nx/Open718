...
public class ContractHandler implements Serializable {
	...
	private static final long serialVersionUID = 1; // Don't change this, else
													// RIP..

	private static final transient int[][] npcs = { { 50, 5, 12, 10, 30 }, // KBD
			{ 8133, 8, 15, 4, 15 }, // Corp
		...
		if (killer.getContract() != null) {
			...
			if (id == killer.getContract().getNpcId()) {
				...
				if (killer.getContract().getKillAmount() >= 1) {
					killer.sm(Colors.GOLDEN + "<shad=292421>You have " + killer.getContract().getKillAmount() + "x " + npcName + " left to kill.");
				}
				if (killer.getContract().getKillAmount() <= 0) {
					...
					killer.sm(Colors.GREEN + "You have completed your reaper task; your reward has been added!");
