...
public class GrandExchangeManager implements Serializable {
	...
	public void openHistory() {
		if (!canUse()) {
			player.sm("Ironmen cannot use the Grand Exchange.");
	...
	public void openGrandExchange() {
		if (player.isIronman()) {
			player.getPackets().sendGameMessage(Color.MAROON, "Ironmen cannot use the grand exchange");
			player.getPackets().sendSound(4039, 1, 1);
			return;
		}
	...
	public void editAmount() {
		...
		player.getPackets().sendInputIntegerScript("Enter the quantity:");
	}
	...
	public void editPrice() {
		if (getType() == -1)
			return;
		if (getItemId() == -1) {
			player.sm("You must choose an item first.");
			player.getPackets().sendSound(4039, 1, 1);
			return;
		}
		player.getTemporaryAttributtes().put("edit_price", Boolean.TRUE);
		player.getPackets().sendInputIntegerScript("Enter the price:");
	}
	...
	public void abortOffer(int slot) {
		...
		player.sm("Please be aware that your offer may have already been completed.");
	}
	...
	public void setExtraDetails(int id) {
		...
		if (getType() == 0) { //Buy
			if (UnlimitedGEReader.itemIsLimited(id)) {
				player.getPackets().sendIComponentText(105, 143, 
						ItemExamines.getExamine(new Item(id)) + "<br> <br>Quantity: Unlimited.");
			} else {
	...
	public void setItem(Item item, boolean sell) {
		if (item.getId() == Shop.COINS || !ItemConstants.isTradeable(item) || item.getName().contains("charm")
				 || item.getId() == 11640) {
			player.sm("This item cannot be traded on the Grand Exchange.");
			player.getPackets().sendSound(4039, 1, 1);
			return;
		}
		if (item.getId() == 18832)
	...
	public void confirmOffer() {
		...
		if (itemId == -1) {
			player.sm("You must choose an item to " + (buy ? "buy" : "sell") + "!");
			player.getPackets().sendSound(4039, 1, 1);
			return;
		}
		int amount = getAmount();
		if (amount == 0) {
			player.sm("You must choose the quantity you wish to " + (buy ? "buy" : "sell") + "!");
			player.getPackets().sendSound(4039, 1, 1);
			return;
		}
		...
		if (pricePerItem != 0) {
			if (amount > 2147483647 / pricePerItem) { // TOO HIGH
				player.sm("You do not have enough coins to cover the offer.");
				player.getPackets().sendSound(4039, 1, 1);
				return;
			}
		}
		if (buy) {
			...
			if (player.getInventory().getCoinsAmount() < price) {
				...
				player.getMoneyPouch().sendDynamicInteraction(price, true);
		} else {
			int inventoryAmount = getItemAmount(new Item(itemId));
			if (amount > inventoryAmount) {
				player.sm("You do not have enough of this item in your inventory.");
				player.getPackets().sendSound(4039, 1, 1);
				return;
			}
