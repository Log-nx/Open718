...
public class Offer extends Item {
	...
	public void sendUpdateWarning() {
		...
		if (isCompleted()) {
			if (buying)
				owner.sm("<col=00BFFF><shad=000000>Grand Exchange: Finished buying "+Utils.getFormattedNumber(amount)+" x "+getName()+".");
			else
				owner.sm("<col=00BFFF><shad=000000>Grand Exchange: Finished selling "+Utils.getFormattedNumber(amount)+" x "+getName()+".");
		} else {
			if (buying)
				owner.sm("<col=00BFFF><shad=000000>Grand Exchange: Bought "+Utils.getFormattedNumber(totalAmountSoFar)+"/"+Utils.getFormattedNumber(amount)+" x "+getName()+".");
			else
				owner.sm("<col=00BFFF><shad=000000>Grand Exchange: Sold "+Utils.getFormattedNumber(totalAmountSoFar)+"/"+Utils.getFormattedNumber(amount)+" x "+getName()+".");
		}
	...
	public boolean collectItems(int slot, int option) {
		...
		if (freeSlots < 1) {
			owner.sm("You need more inventory space to be able to do this!");
		...
		update();
		Logger.logGrandExchange(owner, item);
		return true;
	}
