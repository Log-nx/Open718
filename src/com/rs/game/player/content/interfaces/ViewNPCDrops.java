...
public class ViewNPCDrops {
	...
	public static void changeSortType(Player player, int npcId) {
		...
		player.getPackets().sendGameMessage("You are now sorting by <col=FF0000>"+ sortTypes[player.getDropSortingType()] + "</col>.", true);
	...
	public static void sendDropRateMessage(Player player, Item item, int npcId, int slotId) {		
		player.getPackets().sendGameMessage("<col=FF0000>"+Item.getItemName(item) + "</col> has a <col=FF0000>" + (getDropRate(item.getId(), npcId) + player.getDropRateModifier() >= 100 ? "100" : getDropRate(item.getId(), npcId) * player.getDropRate()) + "%</col> chance of being dropped by the " + NPCDefinitions.getNPCDefinitions(npcId).name + ".", true);
	}
	...
	public static void sendPriceCheckMessage(Player player, Item item) {
		...
		player.getPackets().sendGameMessage("The " + item.getDefinitions().getName() + " has a market value of <col=FF0000>" + Utils.getFormattedNumber(value) + "</col> coins.", true);
	}
