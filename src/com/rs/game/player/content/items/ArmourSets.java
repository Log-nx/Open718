...
public class ArmourSets {
	...
	public enum Sets {
		...
		RUNE_GTRIM_SK(11900, 2619, 2615, 3672, 2621),
		...
		THIRDAGEMAGE(11862, 10342, 10338, 10340, 10344),
		BARROWSAHRIM(11846, 4708, 4710, 4712, 4714),
		BARROWSDHAROK(11848, 4716, 4718, 4720, 4722),
		BARROWSGUTHAN(11850, 4724, 4726, 4728, 4730),
		BARROWSKARIL(11852, 4732, 4734, 4736, 4738),
		BARROWSTORAG(11854, 4745, 4747, 4749, 4751), 
		...
		BLUEMYSTICROBES(11872, 4089, 4091, 4093, 4095, 4097),
		LIGHTMYSTICROBES(11960, 4109, 4111, 4113, 4115, 4117),
		DARKMYSTICROBES(11962, 4099, 4101, 4103, 4105, 4107),
		INFINITYROBES(11874, 6918, 6916, 6924, 6920, 6922),
		SPLITBARK(11876, 3385, 3387, 3389, 3391, 3393),
		ENCHANTEDROBES(11902, 7398, 7399, 7400),
		BLUEWIZROBE_T(11904, 7396, 7392, 7388),
		BLUEWIZROBE_G(11906, 7394, 7390, 7386),
		LEATHER_T(11908, 7364, 7368),
		LEATHER_G(11910, 7362, 7366),
		GREENDHIDE_T(11912, 7372, 7380),
		GREENDHIDE_G(11914, 7370, 7378),
		BLUEDHIDE_T(11916, 7376, 7384),
		BLUEDHIDE_G(11918, 7374, 7382),
		GUTHIXRANGE(11920, 10382, 10378, 10380, 10376),
		SARADOMINRANGE(11922, 10390, 10386, 10388, 10384),
		ZAMORAKRANGE(11924, 10374, 10370, 10372, 10368),
		...
		DAGONHAIROBES(14525, 14499, 14497, 14501),
		THIRDAGEPRAYER(19580, 19314, 19317, 19320, 19311, 19308),
		BANDOSRANGE(19582, 19457, 19453, 19455, 19451),
		ANCIENTRANGE(19584, 19449, 19445, 19447, 19443),
		ARMADYLRANGE(19586, 19465, 19461, 19463, 19459), 
		ARMADYLRUNE_LG(19588, 19422, 19413, 19416, 19425),
		ARMADYLRUNE_SK(19590, 19422, 19413, 19414, 19425),
		BANDOSRUNE_LG(19592, 19437, 19428, 19431, 19440),
		BANDOSRUNE_SK(19594, 19437, 19428, 19434, 19440),
		ANCIENTRUNE_LG(19596, 19407, 19398, 19401, 19410),
		ANCIENTRUNE_SK(19598, 19407, 19398, 19404, 19410),
		BARROWSAKRISAE(21768, 21736, 21744, 21752, 21760),
		ROYALDHIDE(24386, 24382, 24379, 24376);
	...
	public static void exchangeSet(Player player, int slot, int id) {
		...
		if (item == null || item.getId() != id) {
			player.getPackets().sendGameMessage("This isn't an Item Set.");
			return;
		}
		Sets set = getSet(id);
		if (set == null) {
			player.getPackets().sendGameMessage("This isn't a set item, you can't break it up into component parts.", true);
			return;
		}
		if (player.getInventory().getFreeSlots() < set.items.length - 1) {
			player.getPackets().sendGameMessage("You don't have enough inventory space for the component parts.", true);
			return;
		}
		player.getInventory().deleteItem(slot, item);
		for (int itemId : set.items)
			player.getInventory().addItem(itemId, 1);
		player.getPackets().sendGameMessage("You sucessfully traded your item set for its components!", true);
	}
	...
	public static void exchangeSet(Player player, int id) {
		...
		for (int itemId : set.items) {
			if (!player.getInventory().containsItem(itemId, 1)) {
				player.getPackets().sendGameMessage("You don't have the parts to make up this set.", true);
				return;
			}
		}
		for (int itemId : set.items)
		...
		player.getPackets().sendGameMessage("You sucessfully traded your item components for a set!", true);
	}
