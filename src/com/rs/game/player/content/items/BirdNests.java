...
public class BirdNests {
	...
	public static void dropNest(Player player) {
		if (Utils.random((player.getPerkManager().hasPerk(PlayerPerks.BIRD_MAN) ? 50 : 100)) <= 1) {
	...
	private static void handleDrop(Player player, Item nest) {
		if (player.getPerkManager().hasPerk(PlayerPerks.BIRD_MAN)) {
			player.getBank().addItem(nest.getId(), nest.getAmount(), true);
			player.getPackets().sendGameMessage(Colors.ORANGE+"<shad=000000>A bird's nest was sent "
					+ "to your bank thanks to the Bird Man perk.", true);
		} else {
			World.addGroundItem(nest, new WorldTile(player), player, true, 60);
			player.sm(Colors.ORANGE+"<shad=000000>A bird's nest falls out of the tree!");
		}
	}
