...
import com.rs.game.SecondsTimer;
...
import com.rs.game.player.content.timers.AntiFireTimer;
import com.rs.game.player.content.timers.AntiPoisonTimer;
import com.rs.game.player.content.timers.OverloadTimer;
import com.rs.game.player.content.timers.PrayerRenewalTimer;
...
public final class Potions {
	...
	public static enum Pot {
		...
		CHOCOLATEY_MILK(new int[] { 1977, 1925 }, Effects.STRENGTH_BOOST),
	...
	public enum Effects {
		...
		ANTIPOISON() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 86000);
				antiPoisonTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 86);
				player.getPackets().sendGameMessage("You are now immune to all kinds of poison.");
				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Poison has been increased to 86 seconds due to the Herb Law Perk", true);
				player.getPackets().sendHideIComponent(3000, 3, false);
				AntiPoisonTimer.startTime(player);
			}
		},
		ANTIPOISON_PLUS() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 518000);
				antiPoisonTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 518);
				player.getPackets().sendGameMessage("You are now immune to all kinds of poison.");
				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Poison has been increased to 16 minutes due to the Herb Law Perk", true);
				player.getPackets().sendHideIComponent(3000, 3, false);
				AntiPoisonTimer.startTime(player);
			}
		},
		ANTIPOISON_PLUS_PLUS() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 720000);
				antiPoisonTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 720);
				player.getPackets().sendGameMessage("You are now immune to all kinds of poison.");
				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Poison has been increased to 24 minutes due to the Herb Law Perk", true);
				player.getPackets().sendHideIComponent(3000, 3, false);
				AntiPoisonTimer.startTime(player);
			}
		},
		SUPER_ANTIPOISON() {
			@Override
			public void extra(Player player) {
				player.addPoisonImmune((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 346000);
				antiPoisonTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 86);
				player.getPackets().sendGameMessage("You are now immune to all kinds of poison.");

				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Poison has been increased to 692 seconds due to the Herb Law Perk", true);
				player.getPackets().sendHideIComponent(3000, 3, false);
				AntiPoisonTimer.startTime(player);
			}
		},
		...
		ANTI_FIRE() {
			...
			public void extra(final Player player) {
				player.addFireImmune((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				antiFireTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				final long current = player.getFireImmune();
				player.getPackets().sendGameMessage("You are now immune to all kinds of dragonfire.");

				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Fire has been increased to 10 minutes due to the Herb Law Perk", true);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (!stop) {
							player.getPackets().sendGameMessage(Colors.RED + "Your antifire potion is about to run out...");
							stop = true;
						} else {
							stop();
							player.getPackets().sendGameMessage(Colors.RED + "Your antifire potion has ran out.</col>");
						}
					}
				}, (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500, 100);
				player.getPackets().sendHideIComponent(3000, 2, false);
				AntiFireTimer.startTime(player);
			}
		},
		SUPER_ANTI_FIRE() {
			@Override
			public void extra(final Player player) {
				player.addSuperAntiFire((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				antiFireTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Anti-Fire has been increased to 10 minutes due to the Herb Law Perk", true);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (!stop) {
							player.getPackets().sendGameMessage(Colors.RED + "Your super antifire potion is about to run out...");
							stop = true;
						} else {
							stop();
							player.getPackets().sendGameMessage(Colors.RED + "Your super antifire potion has run out.");
						}
					}
				}, (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500, 100);
				player.getPackets().sendHideIComponent(3000, 2, false);
		...
		EXTREME_STR_POTION(Skills.STRENGTH) {
			...
			public boolean canDrink(Player player) {
				if (player.getControllerManager().getController() instanceof Wilderness
						|| player.getControllerManager().getController() instanceof CrucibleControler
		...
		OVERLOAD() {
			...
			public boolean canDrink(Player player) {
				if (player.getControllerManager().getController() instanceof Wilderness
						|| player.getControllerManager().getController() instanceof CrucibleControler|| FfaZone.isOverloadChanged(player)) {
					player.sendMessage("You cannot drink this potion here.");
					return false;
				}
				if (player.getEffectsManager().hasActiveEffect(1)) {
					player.sendMessage("You can't drink this right now.");
					return false;
				}
				return true;
			}
			...
			@Override
			public void extra(final Player player) {
				player.setOverloadDelay((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				overloadTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Overload has been increased to 10 minutes due to the Herb Law Perk", true);
				...
				player.getPackets().sendHideIComponent(3000, 0, false);
				OverloadTimer.startTime(player);
			}
		},
		SUPER_PRAYER() {
			@Override
		...
		PRAYER_RENEWAL() {
			@Override
			public void extra(Player player) {
				player.setPrayerRenewalDelay((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);
				prayerRenewalTimer.start((player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW) ? 2 : 1) * 500);

				if (player.getPerkManager().hasPerk(PlayerPerks.HERB_LAW))
					player.getPackets().sendGameMessage("Prayer Renewal has been increased to 10 minutes due to the Herb Law Perk", true);
				player.getPackets().sendHideIComponent(3000, 1, false);
				PrayerRenewalTimer.startTime(player);
			}
		},
		...
		STRENGTH_BOOST(Skills.STRENGTH) {
			...
			@Override
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				return (int) (actualLevel * 1);
			}

			@Override
			public void extra(Player player) {
				player.getPackets().sendGameMessage("You drink the chocolatey milk..", true);
				player.getPackets().sendGameMessage(".. and feel a lot more positive and stronger.", true);
			}
		},
	...
	public static SecondsTimer overloadTimer = new SecondsTimer();
	public static SecondsTimer prayerRenewalTimer = new SecondsTimer();
	public static SecondsTimer antiPoisonTimer = new SecondsTimer();
	public static SecondsTimer antiFireTimer = new SecondsTimer();
	...
	public static boolean pot(Player player, Item item, int slot) {
		...
		player.setNextAnimation(new Animation(player.isUnderCombat() ? 18003 : 18000));
		...
		if (pot.isFlask() || pot.isPotion()) {
			player.getPackets().sendGameMessage(pot.effect.drinkMessage != null ? pot.effect.drinkMessage
					: "You drink some of your "
							...
							+ ".",
					true);
			player.getPackets().sendGameMessage(dosesLeft == 0
