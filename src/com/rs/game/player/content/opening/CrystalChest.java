...
public class CrystalChest {
	...
	private static Item[] uncommonRewardsI = { 
			new Item(4153, 1), new Item(10564), new Item(10589),
	...
	private static Item[] rareRewardsI = { 
			new Item(6914, 1), new Item(20440), new Item(20444), new Item(20436),
	...
	private static Item[] rareExtraRewardsI = {
			new Item(4860), new Item(4866),
	...
	public static void addCrystalReward(Player player) {
		...
		if (uncommonRewards > 65 && rareRewards > 100) {
			rewards = new Item[] { rareRewardsI[Utils.random(rareRewardsI.length)] };
		} else if (commonRewards > 15 && uncommonRewards > 50) {
			rewards = new Item[] { uncommonRewardsI[Utils.random(uncommonRewardsI.length)] };
		} else if (commonRewards < 40 && uncommonRewards > 40) {
			rewards = new Item[] { commonRewardsI[Utils.random(commonRewardsI.length)] };
		} else {
			rewards = new Item[] { commonRewardsI[Utils.random(commonRewardsI.length)] };
		}
		if (Utils.random(300) > 275) {
			rewards = new Item[] { rareExtraRewardsI[Utils.random(rareExtraRewardsI.length)] };
		}
	...
	public static void handleChestExamine(final WorldObject object,
			Player player) {
	...
	public static void openChest(WorldObject object, final Player player) {
		if (player.getInventory().containsItem(989, 1) && !player.isLocked()) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					if (player.getPerkManager().hasPerk(PlayerPerks.DOUBLE_KEYS)) {
						addCrystalReward(player);
						addCrystalReward(player);
						player.setNextGraphics(new Graphics(5));
						player.chest++;
						player.chest++;
						player.getPackets().sendGameMessage("You've now opened the chest "+player.chest+ " times, and you find some treasure..");
					}
					else
					addCrystalReward(player);
					player.setNextGraphics(new Graphics(5));
			...
			player.getPackets().sendGameMessage("You need a crystal key to open the chest.");
		}
	}
	
	public static boolean keyCreation(Player player) {
		if (player.getInventory().containsItem(987, 1) && player.getInventory().containsItem(985, 1)) {
			player.getInventory().deleteItem(987, 1);
			player.getInventory().deleteItem(985, 1);
			player.getInventory().addItemDrop(989, 1);
			player.sm("You are now making yourself a splendid crystal key.");
		}
		return false;
	}
	...
	public static void openRewardsInterface(Player player) {
		...
		int number = 0;
		for (Item reward : commonRewardsI) {
			if (reward.getId() == 995)
				continue;
		...
		for (Item reward : uncommonRewardsI) {
			if (reward.getId() == 995)
				continue;
			String name = reward.getName() + (reward.getDefinitions().isNoted() ? " - noted" : "");
		...
		for (Item reward : rareRewardsI) {
			if (reward.getId() == 995)
				continue;
			String name = reward.getName() + (reward.getDefinitions().isNoted() ? " - noted" : "");
			player.getPackets().sendIComponentText(275, number + 11, ""+Colors.RED+name);
			number++;
		}
		for (Item reward : rareExtraRewardsI) {
			if (reward.getId() == 995)
				continue;
			String name = reward.getName() + (reward.getDefinitions().isNoted() ? " - noted" : "");
			player.getPackets().sendIComponentText(275, number + 11, ""+Colors.RED+name);
