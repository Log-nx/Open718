...
private static final long serialVersionUID = -5833463661237303707L;
	...
	public void handleLogout(Player player) {
		...
		if (player.hasOngoingInvite == true && newPartner != null) {
			...
			newPartner.sm("<col=C43140>" + player.getDisplayName() + " has logged out, your invite request has been deleted.</col>");
		}
		resetPartner(player);
	}
	...
	public void sendInvite(final Player player) {
		if (player.isIronman()) {
			player.sm("Ironmen cannot do Co-op slayer.");
			return;
		}
	...
	public void handleInviteButtons(Player invited, int interfaceId, int componentId) {
		...
		if (interfaceId == 1310) {
			if (newPartner == null) {
				...
				invited.sm("<col=C43140>Your potential partner has not been found, disbanding party.</col>");
				return;
			}
			switch (componentId) {
			case 0:
			...
			case 1:
				...
				newPartner.sm("<col=C43140>Your invitation has been declined.</col>");
				invited.setSlayerHost(null);
	...
	public void handleCoOpSlayerInterface(Player player, int componentId) {
		...
		switch (componentId) {
		case 20:
			player.sm("Use an Enchanted gem, a Slayer helmet or a Ring of slaying on the player to invite!");
			break;
		case 22:
			if (player.hasOngoingInvite == false) {
				player.sm("You have not been invited by anyone yet!");
				return;
			}
			player.getInterfaceManager().sendInterface(1310);
		...
		case 24:
			if (player.hasGroup == false) {
				player.sm("You're not in a Slayer party at the moment.");
				return;
			}
			player.sm("<col=C43140>You have left your current Slayer party.</col>");
			...
			if (World.containsPlayer(host)) {
				newPartner.sm("<col=C43140>Your Slayer party has been disbaned.</col>");
				newPartner.setSlayerInvite(null);
	...
	public void resetPartner(Player player) {
		player.sm("<col=C43140>Your Slayer party has been disbanded.</col>");
		player.hasHost = false;
		player.hasGroup = false;
