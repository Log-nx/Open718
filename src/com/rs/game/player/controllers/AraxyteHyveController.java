...
public class AraxyteHyveController extends Controller {
    ...
    public boolean login() {
    	removePlayer(player);
    	player.setNextWorldTile(new WorldTile(4582, 6267, 1));
    	return true;
    }
    ...
    public boolean processItemTeleport(WorldTile toTile) {
    	if (araxxor != null) {
	    	player.getPackets().sendGameMessage(Colors.DARK_RED+"You can not leave the Araxyte Hyve this way; you must die or win.");
	    	return false;
    	}
    	return true;
    }
		...
		if (object.getId() == 91500) {
			if (player.getX() >= 4489) {
				player.getDialogueManager().startDialogue("SimpleMessage", 
						Colors.DARK_RED+"The Webbed entrance is closed from this side.");
			}
			return false;
		}
		if (object.getId() == 91673) {
			if (!OwnedObjectManager.isPlayerObject(player, object)) {
				player.getPackets().sendGameMessage("It is not your loot.");
				return false;
			}
    ...
    public static void startFight() {
    	WorldTasksManager.schedule(new WorldTask() {
    	    ...
    	    public void run() {
	    		...
	    		if (stage == 0) {
	    			araxxor = new AraxyteNPC(19463, new WorldTile(4593, 6265, 1));
