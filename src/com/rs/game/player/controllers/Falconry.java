...
public class Falconry extends Controller {
	...
	public void start() {
		...
		player.getEquipment().getItems().set(Equipment.SLOT_WEAPON, new Item(10024));
	...
	public boolean processNPCClick1(final NPC npc) {
		...
		if (npc.getDefinitions().name.toLowerCase().contains("kebbit")) {
			if (!(player.getEquipment().getWeaponId() == 10024)) {
				// if (player.getTemporaryAttributtes().get("falconReleased") !=
				// null) {
				player.getDialogueManager().startDialogue("SimpleMessage",
						"You cannot catch a kebbit without wearing your falcon.");
				return false;
			}
	...
	private boolean proccessFalconAttack(NPC target) {
		...
		if ((!player.clipedProjectile(target, maxDistance == 0)) || distanceX > size + maxDistance
			...
			if (!player.calcFollow(target, 2, true, false))
				return true;
		}
		return true;
	}
