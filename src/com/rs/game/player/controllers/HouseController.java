...
import com.rs.game.player.actions.construction.House.RoomReference;
import com.rs.game.player.actions.construction.HouseConstants;
import com.rs.game.player.actions.construction.HouseConstants.Builds;
import com.rs.game.player.actions.construction.TabletMaking;
...
import com.rs.game.player.actions.prayer.BoneOffering;
...
public class HouseController extends Controller {
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId()) {
		...
		} else if (HouseConstants.Builds.CHARTS.containsObject(object)) { 
			player.getDialogueManager().startDialogue("SimpleMessage", "'A spotter's guide to demons.'");
			...
			// TODO ItemTransportation.transportationDialogue(player, new Item(1712, 1), false);
			return false;
			...
			if (object.getId() == 13214) {
				...
				//player.getPackets().sendMusicEffectOld(90); //TODO music effects
				player.setNextAnimation(new Animation(3674));
			}
			...
			if (house.isBuildMode()) {
				player.getPackets().sendGameMessage("You cannot do this while in building mode.");
				return false;
			} else if (!player.getInventory().containsOneItem(590) || object.getId() >= HouseConstants.HObject.INSCENCE_BURNER.getId() && object.getId() <= HouseConstants.HObject.MARBLE_BURNER.getId() && !player.getInventory().containsItem(251, 1)) {
			...
			if (object.getRotation() == 0) {
				player.useStairs(4853, new WorldTile(player.getX() == object.getX() ? object.getX() - 1 : object.getX(), object.getY(), object.getPlane()), 2, 3);
			} else if (object.getRotation() == 2) {
				player.useStairs(4853, new WorldTile(player.getX() == object.getX() ? object.getX() + 1 : object.getX(), object.getY(), object.getPlane()), 2, 3);
			} else if (object.getRotation() == 1) {
				player.useStairs(4853, new WorldTile(object.getX(), player.getY() == object.getY() ? object.getY() + 1 : object.getY(), object.getPlane()), 2, 3);
			} else {
				player.useStairs(4853, new WorldTile(object.getX(), player.getY() == object.getY() ? object.getY() - 1 : object.getY(), object.getPlane()), 2, 3);
			}
			...
			Magic.sendTeleportSpell(player, -1, -1, -1, -1, 0, 0, HouseConstants.PORTAL_COORDINATES[object.getId() - (13615 + 7 * ((object.getId() - 13615) / 7))], 0, true, Magic.OBJECT_TELEPORT);
			return false;
		}
		return false;
	}
	...
	//TODO replace with processbutton click to disable removing gloves
	/**@Override
		...
		if (interfaceId == 182) {
		    ...
		    if (componentId == 6 || componentId == 13) {
				if (!player.hasFinished()) {
				    player.getPackets().sendGameMessage(Colors.RED+"CAUTION: Please leave your house for security meassures.");
				    return false;
				}
			    return false;
		    }
						...
						if (count == (isTeleportTrap ? 8 : 3)) {
							if (slot == 0 || slot == 1) {
								...
								if (obliette != null) {
									...
									player.useStairs(isPool ? 3641 : 3640, Utils.getFreeTile(house.getCenterTile(obliette), 1), 0, 2, null);
								}
							}
						}
					} else {
	...
	public boolean sendDeath() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					player.setNextAnimation(new Animation(15238));
				} else if (loop == 1) {
