...
public class RunespanControler extends Controller {
	...
	public static WorldTile MEDIUM_LEVEL = new WorldTile(3994, 6105, 1);
	public static WorldTile HIGHER_LEVEL = new WorldTile(4298, 6036, 1);
	 ...
	 * Climbing
	 */
	public static WorldTile LOWER_LEVEL_CLIMB = new WorldTile(3957, 6106, 1);
	public static WorldTile MEDIUM_LEVEL_CLIMB = new WorldTile(4107, 6043, 1);
	public static WorldTile HIGHER_LEVEL_CLIMB = new WorldTile(4296, 6039, 1);
	...
	public static void enterRunespan(final Player player, boolean high) {
		//player.useStairs(-1, high ? LOWER_LEVEL : LOWER_LEVEL, 0, 2);
	...
	public void start() {
		...
		player.getDialogueManager()
				.startDialogue(
						"SimpleMessage",
						"Welcome To the Minigame called RuneSpan.",
						"Find the ladders, if you'd like to get to the higher floors.");
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 70508) { //1st. up
			if (player.getSkills().getLevel(Skills.RUNECRAFTING) < 35) {
			    player.getPackets().sendGameMessage("You don't have enough runecrafting skills to climb up, you need atleast a level of 35.");
			    return false;
		} else {
			Magic.ClimbUpEmote(player, 4107, 6043, 1);
		}
		} if (object.getId() == 70510) { //2st, up
			if (player.getSkills().getLevel(Skills.RUNECRAFTING) < 85) {
			    player.getPackets().sendGameMessage("You don't have enough runecrafting skills to climb up, you need atleast a level of 85.");
			    return false;
		} else {
			Magic.ClimbUpEmote(player, 4296, 6039, 1);
		}
		}else if (object.getId() == 70509) { //2st, down
			Magic.ClimbDownEmote(player, 3957, 6106, 1);
			return true;
		}else if (object.getId() == 70511) { //3rd, down.
						Magic.ClimbDownEmote(player, 4107, 6043, 1);
						return true;
		} else if (object.getId() == 70507) {
		...
		if(object.getId() == 70508) {
			player.useStairs(16668, MEDIUM_LEVEL_CLIMB, 4, 5); 
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
			...
			player.useStairs(16668, HIGHER_LEVEL_CLIMB, 4, 5); 
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(-1));
