...
import com.rs.game.player.actions.construction.ConveyorBeltHopper;
import com.rs.game.player.actions.construction.CutPlank;
import com.rs.game.player.actions.construction.HouseConstants;
import com.rs.game.player.actions.construction.PlanksTake;
import com.rs.game.player.actions.construction.Sawmill;
import com.rs.game.player.actions.construction.StackOfLogs;
...
public class SawmillController extends Controller {
				...
				for (int i = 0; i < INVESTIGATE_COMPONENT_IDS.length; i++) {
					if (componentId == INVESTIGATE_COMPONENT_IDS[i] + 1) {
						if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
							...
							player.getTemporaryAttributtes().put("PlankWithdraw", i);
							player.getPackets().sendInputIntegerScript("Enter amount:");
						}
						return false;
					}
	...
	public void addPlank(int type, int amount) {
		...
		player.getVarBitManager().sendVarBit(type == 5 ? 5008 : 4216 + type, job.planks[type]);
	}
	...
	private void refreshVars() {
		...
		if (job == null)
			return;
		player.getVarBitManager().sendVarBit(5007, job.jobDetails == Jobs.QUICK ? 0 : 1);
		for (int i = 0; i < job.jobDetails.planks.length; i++)
			player.getVarBitManager().sendVarBit(4208 + i, ((job.jobDetails.planks[i] - 5) / 5));
		for (int i = 0; i < job.planks.length; i++)
			player.getVarBitManager().sendVarBit(i == 5 ? 5008 : 4216 + i, job.planks[i]);
	}
	...
	public void sendInterfaces() {
		if (getJob() != null)
			player.getInterfaceManager().sendInterface(766);
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 46317) {
			if (player.getInventory().getFreeSlots() == 0) {
				player.getPackets().sendGameMessage("This crate is full of saws, but you don't have enough space to take one.");
				return false;
			}
			player.lock(2);
		...
		if (object.getId() == 46297) {//take logs
			...
			player.getActionManager().setAction(new StackOfLogs(Integer.MAX_VALUE));
			return false;
		}
		if (object.getId() == 46304) {
			...
			player.getActionManager().setAction(new ConveyorBeltHopper(Integer.MAX_VALUE, this));
			return false;
		}
		if (object.getId() == 46309) {
			...
			player.getActionManager().setAction(new PlanksTake(Integer.MAX_VALUE, this));
			return false;
		}
		...
		if (object.getId() == 46307 && object.getX() == 3311 && object.getY() == 3491) {
			if (Sawmill.hasPlanksOrLogs(player)) {
				player.getDialogueManager().startDialogue("SimpleNPCMessage", Sawmill.OVERSEER, "Oi! That's our wood you've got there! Give it  back!");
				return false;
			}
			player.lock(2);
	...
	public boolean isPlanksFull() {
		return planks >= (player.isDonator() ? 126 : 100);
	}
	...
	private void refreshPlanks() {
		player.getVarBitManager().sendVarBit(4214, planks);
	}
