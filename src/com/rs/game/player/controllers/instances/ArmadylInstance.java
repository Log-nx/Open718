...
import com.rs.game.npc.instances.godwars.armadyl.InstanceArmadyl;
import com.rs.game.npc.instances.godwars.armadyl.InstanceFlightKilisa;
import com.rs.game.npc.instances.godwars.armadyl.InstanceFlockleaderGeerin;
import com.rs.game.npc.instances.godwars.armadyl.InstanceWingmanSkree;
...
public class ArmadylInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(2835, 5293, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("ArmadylInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(351, 661, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(18), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(16, 18));
	...
	public void startWave() {
		try {
		new InstanceArmadyl(6222, getSpawnTile(0));
		new InstanceFlightKilisa(6227, getSpawnTile(0));
		new InstanceFlockleaderGeerin(6225, getSpawnTile(0));
		new InstanceWingmanSkree(6223, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
