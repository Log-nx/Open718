...
import com.rs.game.npc.instances.dagannoth.InstanceDagannothPrime;
import com.rs.game.npc.instances.dagannoth.InstanceDagannothRex;
import com.rs.game.npc.instances.dagannoth.InstanceDagannothSupreme;
...
public class DagannothKingInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(1912, 4367, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("DagannothKingInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(15, 15);
		RegionBuilder.copyAllPlanesMap(362, 554, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 0, getWorldTileX(4), getWorldTileY(14), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(5, 14));
	...
	public void startWave() {
		try {
		new InstanceDagannothSupreme(2881, getSpawnTile(0));
		new InstanceDagannothPrime(2882, getSpawnTile(1));
		new InstanceDagannothRex(2883, getSpawnTile(2));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(5, 14));
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(4), getWorldTileY(14), 0));						
						startWave();
						startTimer();
