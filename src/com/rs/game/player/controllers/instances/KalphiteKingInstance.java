...
public class KalphiteKingInstance extends Controller {
	...
	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("KalphiteKingInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(368, 217, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 0, getWorldTileX(31), getWorldTileY(10), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(30, 10));
	...
	public void startWave() {
		try {
		new KalphiteKing(16697, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(30, 10));
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(31), getWorldTileY(10), 0));						
						startWave();
						startTimer();
