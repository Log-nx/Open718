...
import com.rs.game.npc.instances.InstanceKalphiteQueen;
import com.rs.game.npc.instances.InstanceLegioPrimus;
...
public class KalphiteQueenInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3444, 9497, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("KalphiteQueenInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(15, 15);
		RegionBuilder.copyAllPlanesMap(433, 1185, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 0, getWorldTileX(44), getWorldTileY(14), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(44, 13));
	...
	public void startWave() {
		try {
		new KalphiteQueen(1158, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(44, 13));
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(44), getWorldTileY(14), 0));						
						startWave();
						startTimer();
