...
import com.rs.game.npc.instances.InstanceKingBlackDragon;
...
public class KingBlackDragonInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3069, 10250, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("KingBlackDragonInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(15, 15);
		RegionBuilder.copyAllPlanesMap(281, 583, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 0, getWorldTileX(25), getWorldTileY(16), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(25, 17));
	...
	public void startWave() {
		try {
		new InstanceKingBlackDragon(50, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(25, 17));
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(25), getWorldTileY(16), 0));						
						startWave();
						startTimer();
