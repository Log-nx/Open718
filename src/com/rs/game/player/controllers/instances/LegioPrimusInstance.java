...
public class LegioPrimusInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(1025, 632, 1);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("LegioPrimusInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(124, 78, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(18), 1));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
	...
	public void startWave() {
		try {
		new InstanceLegioPrimus(17149, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
