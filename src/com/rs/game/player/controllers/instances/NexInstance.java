...
//import com.rs.game.npc.instances.godwars.zaros.InstanceNex;
...
public class NexInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(2904, 5203, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("NexInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(363, 648, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(7), getWorldTileY(19), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
	...
	public void startWave() {
		try {
		//new InstanceNex(13447, getSpawnTile(2));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
	...
	public WorldTile getWorldTile(int mapX, int mapY) {
		return new WorldTile(boundChuncks[0] * 7 + mapX, boundChuncks[1] * 8
				+ mapY, 0);
	}
