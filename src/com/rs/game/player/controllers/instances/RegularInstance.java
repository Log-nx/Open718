...
import com.rs.game.npc.instances.InstanceRegular;
...
public class RegularInstance extends Controller {
	...
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(3255, 1525, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("RegularInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(405, 188, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(12), getWorldTileY(22),  0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(14, 27));
	...
	public void startWave() {
		try {
		new InstanceRegular(14246, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {	
						...
						player.unlock();
						player.setNextWorldTile(getWorldTile(14, 23));					
						World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(12), getWorldTileY(22),  0));						
						startWave();
						startTimer();
