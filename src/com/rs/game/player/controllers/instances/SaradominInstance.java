...
import com.rs.game.npc.instances.godwars.saradomin.InstanceBree;
import com.rs.game.npc.instances.godwars.saradomin.InstanceGrowler;
import com.rs.game.npc.instances.godwars.saradomin.InstanceSaradomin;
import com.rs.game.npc.instances.godwars.saradomin.InstanceStarlight;
...
public class SaradominInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(2923, 5258, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("SaradominInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(363, 654, boundChuncks[0], boundChuncks[1], 8);	
	...
	public void startWave() {
		try {
		new InstanceSaradomin(6247, getSpawnTile(0));
		new InstanceStarlight(6248, getSpawnTile(0));
		new InstanceGrowler(6250, getSpawnTile(0));
		new InstanceBree(6252, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
