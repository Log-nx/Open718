...
import com.rs.game.npc.instances.InstanceSunfreet;
...
public class SunfreetInstance extends Controller {
	...
	private int count;
	private WorldTile ORIGINAL_LOCATION =  new WorldTile(1686, 4327, 0);

	public static void enterTrioInstance(Player player) {
		player.getControllerManager().startController("SunfreetInstanceControler", 1);
	}

	@Override
	public void start() {
		boundChuncks = RegionBuilder.findEmptyChunkBound(8, 8);
		RegionBuilder.copyAllPlanesMap(470, 437, boundChuncks[0], boundChuncks[1], 8);	
    ...
    private void startTimer() {
    	WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (count == 1) {
					try {
					World.spawnObject(new WorldObject(28779, 10, 1, getWorldTileX(16), getWorldTileY(21), 0));
					} catch (Throwable t) {
						Logger.handle(t);
					}
				}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					player.reset();
					player.setNextWorldTile(getWorldTile(16, 20));	
	...
	public void startWave() {
		try {
		new InstanceSunfreet(15222, getSpawnTile(0));
		} catch (Throwable t) {
			Logger.handle(t);
		}
	}
