...
public class CombinationsD extends Dialogue {
	...
	public void run(int interfaceId, int componentId)  {
		...
		Object[] data = getCombination(SkillsDialogue.getItem(SkillsDialogue.getItemSlot(componentId)));
		if (data == null)
			return;
		final Combinations combination = (Combinations) data[0];
		final int componentIndex = (int) data[1];
		System.out.println("componentIdx: "+componentIndex+"; quantity: "+quantity+".");
		...
		player.getActionManager().setAction(new Action() {
			...
			public boolean process(Player player) {
				...
				for (Item item : combination.getRequirements()) {
					if (!player.getInventory().containsOneItem(item.getId(), item.getAmount())) {
			...
			public int processWithDelay(Player player) {
				...
				player.getInventory().addItem(combination.getProducedNodes()[componentIndex]);
	...
	public enum Combinations {
		...
		private Object[] parameters; // type, requirements for certain type
		...
		public static Combinations isCombining(int initialNode, int appliedNode) {
			for (Combinations combination : Combinations.values()) {
				if (combination.getInitialNode().getId() == initialNode 
						&& combination.getSecondaryNode().getId() == appliedNode 
						|| combination.getInitialNode().getId() == appliedNode 
						&& combination.getSecondaryNode().getId() == initialNode)
					return combination;
			}
			return null;
		}
