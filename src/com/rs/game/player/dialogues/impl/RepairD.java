...
public class RepairD extends Dialogue {
	...
	public void run(int interfaceId, int componentId)  {
		if (componentId == OPTION_1) {
			...
			if (player.getStatistics().getDungeoneeringTokens() >= prices[1])
				player.getStatistics().setDungeoneeringTokens(player.getStatistics().getDungeoneeringTokens() - (-prices[1]));
			else {
				sendDialogue("You don't have enough tokens, you need " + Utils.getFormattedNumber(prices[1]) + " tokens to repair this item.");
				return;
			}
			repairItem();
		}
