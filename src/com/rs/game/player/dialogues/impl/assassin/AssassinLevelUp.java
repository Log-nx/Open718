...
public final class AssassinLevelUp extends Dialogue {
	...
	public static final int[] SKILL_LEVEL_UP_MUSIC_EFFECTS = { 37, 37, 37, 37, 37 };
	...
	public void start() {
		...
		player.sm("You've just advanced a" + (name.startsWith("A") ? "n" : "") + " " + name + " level! You have reached level " + level + ".");
	...
	public static void sendNews(Player player, int skill, int level) {
		...
		if (player.inform99s) {
			World.sendWorldMessage("<img=6><col=ff8c38>News: " + player.getDisplayName() + " has achieved " + level + " " + Skills.SKILL_NAME_ASSASSIN[skill] + ".", false, false);
		}
	}
