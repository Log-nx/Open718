...
import com.rs.game.player.content.overrides.CosmeticsHandler.Cosmetics;
import com.rs.game.player.dialogues.Dialogue;
import com.rs.utils.Colors;
import com.rs.utils.Utils;

public class CosmeticStoreD extends Dialogue {

	public static final int[] REMOTE_FARM_INTERFACE_NAMES = { 30, 32, 34, 36, 38, 49, 51, 53, 55, 57, 59, 62, 64, 66,
			68, 70, 72, 74, 76, 190, 79, 81, 83, 85, 88, 90, 92, 94, 97, 99, 101, 104, 106, 108, 110, 115, 117, 119,
	...
	public void start() {
		...
		for (Cosmetics c : Cosmetics.values()) {
			if (c == Cosmetics.HIDE_ALL || c.getPrice() == -1
					|| (keyWord != null && !c.getName().toLowerCase().contains(keyWord)))
				continue;
	...
	private void sendComponents() {
		...
		String availability = CosmeticsHandler.isRestrictedItem(player, option.getCosmetics())
				? Colors.RED + "Buy Costume."
		...
		for (int i = 0; i < componentsCount; i++) {
			...
			availability = CosmeticsHandler.isRestrictedItem(player, option.getCosmetics())
					? Colors.RED + "Buy Costume."
					: Colors.GREEN + "Available.";
		...
		player.getPackets().sendIComponentText(1082, 11,
				Colors.RED + "Red = Not Available" + Colors.WHITE + ".   " + Colors.GREEN + "Green = Available"
						+ Colors.WHITE + ".<br>" + Colors.WHITE
						+ "Click On The \"Buy Costume\" To Buy It. Diccus Coins Amount: "
						+ player.getStatistics().getRuneCoins() + ".");
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		case 1188:// options dialogue
			...
			case OPTION_1:
				int price = option.getPrice();
				if (player.getStatistics().getRuneCoins() < price) {
					sendDialogue("You don't have enough Diccus Coins to buy this. You currently have "
							+ player.getStatistics().getRuneCoins() + " Diccus Coins.");
					return;
				}
				player.getStatistics().setRuneCoins(player.getStatistics().getRuneCoins() - price);
	...
	private void sendOptions() {
		...
		sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE,
				price == 0 ? "Unlock costume" : ("Buy costume" + " (" + price + ")"), "Preview costume", "Cancel");
	}
