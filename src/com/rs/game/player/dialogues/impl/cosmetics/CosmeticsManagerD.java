...
public class CosmeticsManagerD extends Dialogue {
	...
	@Override
	public void start() {
		sendOptionsDialogue("Choose an Option", "Open Cosmetics", "Open Cosmetics store", "Save Current Cosmetic",
				"Toggle Search Option", Colors.RED + "More Options");
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		case -1:
			if (componentId == OPTION_1) {
				...
				sendOptionsDialogue("Choose an Option", "Toggle filter locked cosmetics", "Reset Cosmetic",
						"Color Costume", "Reset Costume Color", Colors.RED + "More Options");
				stage = 2;
			}
			break;
		case 2:
			if (componentId == OPTION_1) {
				...
				player.getPackets().sendGameMessage("You will now see "
						+ (!player.isFilterLocked() ? "all the cosmetics" : "only the cosmetics you have unlocked")
						+ ".");
			} else if (componentId == OPTION_2) {
				end();
		...
		case 3:
			if (componentId == OPTION_1) {
				...
				stage = -1;
				sendOptionsDialogue("Choose an Option", "Open Cosmetics", "Open Cosmetics store",
						"Save Current Cosmetic", "Toggle Search Option", Colors.RED + "More Options");
			}
			break;
		}
