		...
		player.getActionManager().setAction(new Action() {
			...
			public int processWithDelay(Player player) {
				...
				player.getInventory().deleteItem(
						new Item(INGREDIENT[componentIndex], player.getInventory().getAmountOf(INGREDIENT[componentIndex])));
				player.getInventory().addItem(
						new Item(PRODUCT[componentIndex], player.getInventory().getAmountOf(INGREDIENT[componentIndex])));
				if (price != 0)
					player.getMoneyPouch().sendDynamicInteraction(price, true, MoneyPouch.TYPE_POUCH_INVENTORY);
				return 0;
			}

			@Override
			public boolean start(final Player player) {
				final int leatherAmount = player.getInventory().getAmountOf(
							...
							player.getDialogueManager()
									.startDialogue(
											...
											"Ahhh... novice mistake, you must bring me at least one "
													+ ItemDefinitions
															.getItemDefinitions(INGREDIENT[componentIndex])
															.getName()
															.toLowerCase()
													+ " in order to tan "
													+ ItemDefinitions
															.getItemDefinitions(PRODUCT[componentIndex])
															.getName()
															.toLowerCase()
													+ ".");
						}
						...
						public void run() {
							player.getDialogueManager()
									.startDialogue(
											"SimpleNPCMessage",
											npcId,
											"Young traveller! You aren't skilled enough to make THIS yet. Come back when you have a Crafting level of at least "
													+ levelReq + ".");
						}
					});
					return false;
				...
				final int maximumQuantity = price == 0 ? requestedAmount
						: player.getMoneyPouch().getCoinsAmount() / price;
