	...
	public void run(int interfaceId, int componentId)  {
		if (interfaceId == 1183 && componentId == 9) {
			if (player.getInventory().getFreeSlots() >= 1) {
			    if (player.getStatistics().getDungeoneeringTokens() < item.getCost())
			    	return;
			    if (player.getSkills().getLevel(Skills.DUNGEONEERING) < item.getRequirement())
			    	return;
				player.getStatistics().setDungeoneeringTokens(player.getStatistics().getDungeoneeringTokens() - item.getCost());
