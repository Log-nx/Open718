...
public class LegioPrimus extends Dialogue {
	

	public void setPerma(boolean bool) {
		player.setPermaLegioPrimus(bool);
	}
	private String getString = "LegioPrimusInstance";
	public void startController() {
		player.getControllerManager().startController("LegioPrimusInstance");	
	}
	public LegioPrimus() {
	}
	...
	public void start() {
		stage = 100;
		player.instanceDialogue = "LegioPrimus";
		player.setOutside(new WorldTile(1025, 631, 1));
		stage = 1;
	...
	public void run(int interfaceId, int componentId)  {
		boolean getPerma = player.getPermaLegioPrimus();
		if (stage == 1) {	
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(1023, 632, 1));
		...
		stage = 9;					
			...
			if (componentId == OPTION_1) {
				...
				if (player.getMoneyPouch().getCoinsAmount() >= 200000) {
					player.setInstancePin(2);
					player.getMoneyPouch().sendDynamicInteraction(200000, true);
					player.sm("200,000 coins were deducted from your inventory.");
					player.sm("Enjoy your instance, the NPCs will stop spawning in an hour.");
					startController();
					end();
				} else {
