...
public class OwnedObjectManager {
	...
	public static boolean removeObjectWithCoords(Player player, WorldObject object) {
		for (Iterator<String> it = player.getOwnedObjectManagerKeys().iterator(); it.hasNext();) {
			...
			if (manager.getCurrentObject().getX() == object.getX() && manager.getCurrentObject().getY() == object.getY() && manager.getCurrentObject().getPlane() == object.getPlane()) {
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						manager.delete();
	...
	public static String addOwnedObjectManager(Player player, WorldObject[] object, long[] cycleTimes) {
		return addOwnedObjectManager(player, object, cycleTimes, null);
	}

	public static String addOwnedObjectManager(Player player, WorldObject[] object, long[] cycleTimes, ProcessEvent event) {
		return new OwnedObjectManager(player, object, cycleTimes, event).managerKey;
	}
