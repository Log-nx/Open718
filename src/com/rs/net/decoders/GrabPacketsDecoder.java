...
public final class GrabPacketsDecoder extends Decoder {
	...
	private final void decodeRequestCacheContainer(InputStream stream,
		...
		if (archiveId < 0)
			return;
		if (indexId != 255) {
			if (Cache.RS3CACHE.getIndexes().length <= indexId
					|| Cache.RS3CACHE.getIndexes()[indexId] == null
					|| !Cache.RS3CACHE.getIndexes()[indexId]
							.archiveExists(archiveId))
				return;
		} else if (archiveId != 255)
			if (Cache.RS3CACHE.getIndexes().length <= archiveId
					|| Cache.RS3CACHE.getIndexes()[archiveId] == null)
				return;
	...
	private final void decodeOtherPacket(InputStream stream, int packetId) {
		...
		if (packetId == 4) {
			...
			stream.skip(11); //for some reason 11bytes instead of 3 on 718+ protocol
	}
}
