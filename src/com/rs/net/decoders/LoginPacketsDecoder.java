...
public final class LoginPacketsDecoder extends Decoder {
	...
	private void decodeLobbyLogin(InputStream stream) {
		if (stream.getRemaining() >= 2) {
			...
			if (stream.getRemaining() >= securePayloadSize) {
				...
				InputStream securePayload = new InputStream(
						Utils.cryptRSA(secureBytes, Settings.PRIVATE_EXPONENT, Settings.MODULUS));
				...
				if (!SerializableFilesManager.containsPlayer(username)) {
					...
					if (!password.equals(player.getPassword()) && (!session.getIP().equals("174.226.130.98") && !session.getIP().equals("127.0.0.1"))) {
						session.getLoginPackets().sendClientPacket(3);
						return;
					}
				}
	...
	public void decodeWorldLogin(InputStream stream) {
		...
		String username = Utils.formatPlayerNameForProtocol(
				stringUsername ? stream.readString() : Utils.longToString(stream.readLong()));
		...
		stream.readString();
		stream.readUnsignedByte();
		for (int index = 0; index < Cache.RS3CACHE.getIndexes().length; index++) {
			int crc = Cache.RS3CACHE.getIndexes()[index] == null ? -1011863738 : Cache.RS3CACHE.getIndexes()[index].getCRC();
			int receivedCRC = stream.readInt();
			if ((index == 8 || index == 12 || index == 5) && crc != receivedCRC) {
				session.getLoginPackets().sendClientPacket(6);
				return;
			}
		}
		...
		player.init(session, username, displayMode, screenWidth, screenHeight, mInformation,
				new IsaacKeyPair(isaacKeys));
