...
import com.rs.game.player.appearance.HintIcon;
...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendConfigByFile1(int fileId, int value, boolean newDefs) {
	...
	public void sendConfigByFile2(int fileId, int value, boolean newDefs) {
	...
	public void sendDuelChallengeRequestMessage(Player p, boolean friendly) {
		sendMessage(101, "wishes to duel with you (" + (p.isOnSpotDueling() ? "on-spot" : "duel arena") + ") ("
	...
	public void sendWorldList(boolean full) {
		...
		if (full) {
			...
			for (int world = 1; world <= WorldList.WORLDS.size(); world++) {
				packet.writeSmart(WorldList.getWorld(world).getCountryId()); // Flag
																				// icon
																				// :3
			...
			for (int world = 1; world <= WorldList.WORLDS.size(); world++) {
				...
				// 0x8)
				packet.writeGJString(""); // Leave empty for country name, or
											// else
				// location but that removes flag image
				// OR REPLACE BY -
				packet.writeGJString(WorldList.WORLDS.get(world).getIP()); // Endpoint
																			// to
																			// connect
																			// to
			}
	...
	public void sendClanInvitation(Player p) {
		sendMessage(117, p.getDisplayName() + " is inviting you to join their clan.", p);
	...
	public void sendItemMessage(int border, int colour, int id, int x, int y, String message) {
		...
		sendGlobalConfig(1700, border); // "border" - Default; 0 - White; 1 -
										// Red; 2 - No Border
