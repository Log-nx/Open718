...
public class CacheSearch {
	...
	private static void searchForNPC(Player player, String npcName) {
		...
		for (int i = 0; i < Utils.getNPCDefinitionsSize(); i++) {
			...
			if (name.toLowerCase().contains(npcName.toLowerCase())) {
				...
				int MAX_RESULTS = 250;
				if (count == MAX_RESULTS) {
					player.getPackets().sendPanelBoxMessage("<col=FF0000>Found over 250 results for " + Utils.formatPlayerNameForDisplay(npcName) + ". Only 250 listed.");
					return;
				}
				player.getPackets().sendPanelBoxMessage("<col=00FFFF>" + Utils.formatPlayerNameForDisplay(name) + "</col> (Id: <col=00FF00>" + i + "</col>)");
			}
		}
	...
	private static void searchForObject(Player player, String objectName) {
		...
		for (int i = 0; i < Utils.getObjectDefinitionsSize(); i++) {
			try {
				...
				if (name.toLowerCase().contains(objectName.toLowerCase())) {
					...
					int MAX_RESULTS = 250;
					if (count == MAX_RESULTS) {
						player.getPackets().sendPanelBoxMessage("<col=FF0000>Found over 250 results for " + Utils.formatPlayerNameForDisplay(objectName) + ". Only 250 listed.");
						return;
					}
					player.getPackets().sendPanelBoxMessage("<col=00FFFF>" + Utils.formatPlayerNameForDisplay(name) + "</col> (Id: <col=00FF00>" + i + "</col>)");
				}
		...
		player.getPackets().sendPanelBoxMessage(
				"<col=FF0000>Found " + count + " results for the object " + Utils.formatPlayerNameForDisplay(objectName) + ".");
	}
